...
this in a 
uniform way, each lens is equipped with a
...
problem~\cite{DBLP:journals/tods/BancilhonS81} that was adapted to
lenses by~\citeN{Matching10} (and, for a slightly different
definition, by~\citeN{matsuda2007btb}), and it is quite simple.  If we think of
the    
...
difficult than the task (already handled by asymmetric lenses) of merging a
view and source that do not match.
\fi
...
\iffull
One may worry that there may be useful transformations which form asymmetric
lenses but not asymmetric lenses with complements, because the view
structure $Y$ and the complement structure $C$ are not orthogonal -- that
is, that $\GET$ is not surjective but rather only produces values of $Y
\times C$ that satisfy some invariant connecting the view $Y$ and the
structure $C$. In such cases, $\PUT$ must be prepared to handle values of $Y
\times C$ that violate this invariant. An example of this, suggested by a
reviewer of an early version of the paper, is one in which the $\GET$
transformation is a tree traversal, producing a list of the tree's elements
as the view and a tree shape (without elements inside) as the complement.
This always produces lists and tree shapes with the same number of holes.
Then $\PUT$ would need to be able to handle being provided with a list and
tree shape whose sizes do not match. However, the corresponding lens without
complement must also face this problem, when its $\PUT$ is provided a tree
and a list whose sizes do not match. So the asymmetric lens with complement
can steal whatever conflict resolution policy the asymmetric lens uses. This
``policy-stealing'' argument is fully general, as we show now.

We can convert back and forth between the two presentations; in particular,
if $(\GET, \PUT, \CREATE)$ are the components of a traditional lens, then we
define a canonical complement by $C=\{f\in Y{\rightarrow}X\mid \forall
y.\;\GET(f(y))=y\}$. We then define the components $\missing'$, $\GET'$, and
$\PUT'$ of an asymmetric lens with complement as $\missing'=\CREATE$ and
$\GET'(x)=(\GET(x),\lambda y.\PUT(y,x))$ and $\PUT'(y,f)=f(y)$. Going the
other way, if $(\aget, \aput, \missing)$ are the components of an asymmetric
lens with complement, we can define a traditional lens by $\aget'(x) =
\mlfst(\aget(x))$ and $\aput'(y,x) = \aput(y,\mlsnd(\aget(x)))$ and
$\acreate(y) = \aput(y,\missing)$.
\fi%full
...
C_Y$---that is, each element of the complement contains some ``private
information from 
...
equation $\putr(x,c) = (y,c)$ to characterize the steady states.  (The
...
complement $c$ from the previous steady state; as a result\bcp{Not clear why
  this follows}, it can be
somewhat delicate to choose a good value for $\missing$. Fortunately, this
value can often be 
calculated compositionally: each of our primitive lenses and lens combinators
specifies one good choice for $\missing$.
...
$\PUT$s---the dates from the left-hand structure and the countries
...
name) from left to right, store the private information (the new
...
input and $(x',z)$ as complement; it constructs a new complement by
...
This law is somewhat controversial: on one hand, some reasonable $\aget$
...
law; but, on the other hand, relying on this law allows one to optimize
chains of successive 
puts and strongly constrains the operation of $\aput$, ruling out some
clearly unsatisfactory $\aput$ implementations. We briefly explore some of the
ways one might generalize this law to the realm of symmetric lenses.
...
in \S\ref{lists} (Defs.~\ref{delala} \& \ref{dedaja}) satisfy these laws.
\fi
...
the challenging issue of {\em alignment}~\cite{Boomerang07,Matching10}. We
\fi
...
Since each lens carries its own complement---which need
...
\ifdissertation
  ...
  \end{align*}
\else
that $      (c_k,c_m) \in R_{km}$, 
that $     k.\putr(x,c_k) = (y_k, c_k')$, 
and that $ m.\putr(x,c_m) = (y_m, c_m')$.
\fi
...
With the basic definitions in hand, our next task is to show how lenses can
be built, either from scratch or by composing other lenses in various ways.
We begin in this section with several relatively simple constructions.
...
%         \[k\op.\putl = k.\putr \sim_R \ell.\putr = \ell\op.\putl\]
    ...
    \item The relation $(c_k,c_\ell) \relR (c_\ell,c_k)$ whenever $c_k \in
        k.C$ and $c_\ell \in \ell.C$ witnesses the
        \hbox{equivalence}.
% We observe
%         that:
%         \begin{align*}
...
\end{theorem}
{\sc Proof idea:} Suppose we have lenses $k \in Z \Lens X$ and $\ell \in Z
...
%
\iffull
See Proposition~13 in~\citeN{AbrTze09} for a proof that these conditions are
...
$k.\putl(z,c_k')=(x,c_k')$ and $\ell.\putl(w,c_\ell')=(y,c_\ell')$,
so that:
...
\LENS{}, that is,
 $\id_X \otimes \id_Y \equiv \id_{X \times Y}$, and
 $(k_1 ; \ell_1) \otimes (k_2 ; \ell_2) \equiv (k_1 \otimes k_2)
; (\ell_1 \otimes \ell_2)$.
\end{lemma}
...
i.e., \else---\fi$\eq(x,x')$ yields $\mlinl \unit$ if $x = x'$ and 
yields $x'$ if not.
\iffull
...
\ifdissertation
  ...
  \end{array}\]
\else
$   \inl_{X,Y}  X \to X \oplus Y $ and
$   \inr_{X,Y}  Y \to X \oplus Y $ and
$   \codiag_X  X \oplus X \to X$
\fi
such that
\iffull
\[
\inl ; (f \oplus g) = f ; \inl \qquad
\inr ; (f \oplus g) = g ; \inr \qquad
...
\iffull
\ifdissertation\else \breakifnearbottom \fi
...
\[\id_X \oplus^f \id_Y \equiv \id_{X+Y} \qquad
...
\begin{align*}
    ...
    \id.\putr(v,c) &= (v,\unit)
\end{align*}
Since $v=v$ and the complements are always related, \ifdissertation this
shows that \fi
...
\[
      v = \mlinl x_0 \qquad
      c_a = (\mlinl c_k, \mlinl c_\ell) \qquad
...
\[
    v = \mlinl x_0 \qquad
    c_a = (\mlinr c_{k'}, \mlinr c_{\ell'}) \qquad
...
\end{align*}
\fi
Again, since $z_0,c_k,c_\ell$ are the same in both equations,
...
X\LIST$ be the bijection between ``unfolded'' lists and lists\footnote{We
    have now introduced two senses of the word ``fold'' (and ``unfold''):
    as a recursion pattern, and as an isomorphism between two
    representations of a recursive data type. Context will make it clear
which of these are meant in what follows.}; $\foldlist$ takes $\mlinl
...
the mapping operation available for general containers.\footnote{Readers
  might wonder whether, since suitable categories of containers have initial
  algebras, they already provide ``the usual fold stuff.''  In fact, they do
  not: the morphisms in the categories of containers are rather restricted
  and require, for instance, any reordering of the positions to be
  independent of the actual data contained in those. Thus, for example, a
  sorting function is not a container morphism and cannot be construed as an
  initial algebra map in the category of containers.}  
 Moreover, the
...
\begin{longenum}
\item We have $a=\acopy_X\sym$ and $b=\id_X$. The singleton
...
\item We have $a=(k;\ell)\sym$ and $b=k\sym;\ell\sym$. The relation
...
\item We have $a=\aconst_x\sym$ and $b=\const_x$. The relation
...
\item We have $a=(k\cdot\ell)\sym$ and $b=k\sym\otimes\ell\sym$. The relation
...
\item We have $a=(k|\ell)\sym$ and $b=(k\sym\oplus^f\ell\sym);\union$.
    Suppose $k \in X \alens Y$ and $\ell \in Z \alens W$.  Define the
following functions:
...
equivalence.
\item We have $a=(\ell\LIST)\sym$ and $b=\map^f(\ell\sym)$. $(\ell^*)\sym.C$
comprises functions $f:Y^*\to X^*$ such that
...
\fi
  ...
  an equivalence.
  \else
 A full account of the machinery necessary to realize this approach has recently been
 given by Johnson and Rosebrugh~\cite{johnson2014spans}. In particular, it is shown there
how lens composition on the level of spans can be faithfully represented using pullbacks and how a notion of lens equivalence may be defined on
spans so that this composition becomes associative.  This notion of
equivalence is finer than the bisimulation-based one that we use. 
  \fi%complement
  \fi%full
