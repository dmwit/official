...
contain information that is not present in the other (Section~\ref{symm}).
...
technical and practical limitation (see Section~\ref{sec:relwork}).
%
...
(Section~\ref{basic}). We then settle some basic questions about products
and sums (Sections~\ref{prod} and~\ref{sumlist}). In particular, it was
previously known 
...
structures such as lists and trees (Section~\ref{iter}).  We first observe
...
all the elements of some data structure (Section~\ref{contain}).  This
...
%% for recursive datatypes such as lists and trees.  Section~\ref{sumlist}
%% gives direct definitions of two variants of a ``list mapping'' constructor
%% (corresponding to the two variants of sum).  Sections~\ref{iter}
%% and~\ref{contain} develop two different ways to obtain the same lens as an
%% instance of a much more general construction.  In Section~\ref{iter} we use
%% the methods of universal algebra \finish{... fill this in ...}.  In
%% Section~\ref{contain} we use Joyal's notion of {\em
...
asymmetric lenses.  Indeed, we can show (Section~\ref{asymm}) that
...
Sections~\ref{sec:relwork} and~\ref{future} discuss related and future
work. 
...
refine the whole framework of symmetric lenses with a notion of {\em delta
  propagation}; see Section~\ref{future}.
...
\subseteq k.C \times \ell.C$ by $R = \{(c,d) \;|\; \apply(k,c,P)=\apply(\ell,d,P)
\mbox{ for all $P$}\}$. By assumption, we have $(k.\missing, \ell.\missing) \in
R$.
...
\dmwit{review the lemma, its proof, and the corollary}
...
$h(\bij_f.\missing) = \ell.\missing$ because all elements of $\ell.C$ are
...
Recalling that $c \in R$ means there are $c_k,c_\ell$ such that
...
so that $\mlfst(k.\putl(\unit,k.\missing))=x$.

Let us show next that $k.\putl \sim_R \const_x.\putl$. Choose $c$ and $y$
such that $\mlfst(k.\putl(\unit,c))=y$ and arbitrary $v\in\Unit$. Clearly,
...
\relRl \ell'.\missing$, we know that $(k.\missing,\ell.\missing)
\relR (k'.\missing,\ell'.\missing)$, that is:
...
\begin{prop}[\LENS{},$\otimes$ is a symmetric monoidal category]
...
symmetric monoidal \iffull categories\else category\fi] 
...
\formartin{from here to the proposition is new}
...
As we saw in Section~\ref{symm}, there is also a forgetful variant of the
...
\iffull
\begin{theorem}[List iteration is well-defined]\label{listiter}
\else
...
% $w \in X_1\LIST \times X_2\LIST \rightarrow \NAT$
given by 
...
gives us $\IT(\ell).\putr(x{\CONS}xs, c{\CONS}cs) = (z',c'{\CONS}cs')$, where 
\[
\begin{array}{lcl}
...
Suppose we have
%
        $\IT(\ell).\putl(z, c\CONS{}cs) = (x\CONS{}xs,c'\CONS{}cs')$, where 
        $(\mlinr(x,z'),c') = \ell.\putl(z,c)$ and 
...
$f_{rl}(b) = \mlfst(\ell.\putl(f_r(b),t_r(i)(b)))$. Inspecting the
...
\end{eqnarray*}
Observing that $k.\aget(f_k(f_\ell(z)))=f_\ell(z)$ because $f_k \in k\sym.C$
and that $f_k(f_\ell(z)) = f_{k\ell}(z)$ because $f_{k\ell} \relR
(f_k,f_\ell)$, that last line becomes
\begin{eqnarray*}
    f_k'(f_\ell'(z'))
...
need to show that $(\ell_r;\ell_l).\putr(x,(f_c,g_c))=(y,(f_{c'},g_{c'}))$.
...
the constructions in Sections~\ref{basic} to~\ref{sumlist} (including lenses
...
monoidal categories 
are in 1-1 correspondence with wiring diagrams and with first-order
...
the lens $\mathit{concat}'$ from Section~\ref{concatprime}:  
\[\begin{array}{@{}l}
...
to a {\em stateful} presentation of lenses.  An upcoming workshop paper will
discuss our work on a Haskell implementation of symmetric lenses in this
vein.  The idea is that the complement
...
As we mentioned in Section~\ref{firstalign}, dealing correctly with
