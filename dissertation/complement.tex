...
which of these are meant in the remainder.}; $\foldlist$ takes $\mlinl
...
the mapping operation available for general containers.\bcp{Do we want this
  argument with the reviewer in the published paper?  If so, perhaps we can
  reword it as ``One might wonder whether...'' or some such?}\footnote{An anonymous referee remarked that suitable categories of containers have initial algebras thus providing ``the usual fold stuff.'' We disagree: the morphisms in the categories of containers are rather restricted and require, for instance, any reordering of the positions to be independent of the actual data contained in those. Thus, for example, a sorting function is not a container morphism and cannot be construed as an initial algebra map in the category of containers.} 
 Moreover, the
