...
\times Y)\LIST \to \partial(X \times Z)\LIST$,\footnote{The symbol $\dputr$ is
  ...
  in~\cite{Focal2005-shortcite,Boomerang07}.} and another for transporting
...
\ifdissertation As in Chapter~\ref{chap:complement}, the \else The \fi
solution is to store a small list, called a {\em complement}, off to the
...
\iffull
we also expect will be small compared to the size of the \replicas in
most cases.
\else
...
presentations of lenses~\cite{Focal2005-shortcite}, is formalized in Theorem
...
\iffull \vspace{-2ex}\begin{defn}[Composition]\  \fi
...
\begin{lemma}
    This lens is well-defined: $\dputr$ and $\dputl$ are homomorphisms, and
    $K$ is preserved.
\end{lemma}
\begin{proof}
...
almost a symmetric monoidal category; the only missing
...
\begin{pf}\ 
    \begin{itemize}
        ...
        \item $\id\oplus\id\equiv\id$
            ...
            We will use the witnessing relation that relates all consistent
            complements:
            \[S = \{(x, c, d, y) \mid  (x, c, y) \in (\id\oplus\id).K
                                 \land (x, d, y) \in \id.K\}\]
            Naming the module in question $X\oplus Y$, it is clear that
            \[(\init_X,(\id\oplus\id).\missing,\id.\missing,\init_Y) \in S\]
            because $\id\oplus\id$ and $\id$ are both lenses. It remains to
            show that $\dputr$ and $\dputl$ preserve $S$. We will show
            the argument for $\dputr$ applied to generators (because the
            monoid is free); the argument for $\dputl$ is symmetric.

            Hence we may assume we have consistent triples $(x,c,y) \in
            (\id\oplus\id).K$ and $(x,d,y) \in \id.K$ and an edit generator
            $\dx$ for which $\dx\;x\Defined$. Both $\id\oplus\id$ and $\id$
            are lenses, so we know that applying their $\dputr$ operations
            to $\dx$ will produce an edit that restores consistency; this
            means that we need only check that they output the \emph{same}
            consistency-restoring edit. Since
            $\id.\dputr(\dx,d)=(\dx,\unit)$, we must show
            $(\id\oplus\id).\dputr(\dx,c)=(\dx,c')$ for some $c'$. This
            property is clearly true by inspecting the cases in the
            definition of the $\oplus$ combinator; one need only observe
            that $\dputr\gen$ outputs $\fail$ only in cases where $\dx\;x$
            is visibly not defined, a contradiction to our assumption that
            it is.
...
\begin{proof}\ 
    ...
    \begin{itemize}
        ...
        \item Define the function $\ml{unzip}$ as follows:
            ...
            of lists and list of tuples, respectively.\endofpf
    \end{itemize}
\end{proof}
\fi
...
\newcommand{\markeqn}[1]{%
  \refstepcounter{mark}%
  \label{ref#1}
  \ifdissertation&\else\hfill\hspace{1em}\fi%
  \mbox{\scriptsize (\arabic{mark})\!\!\!}%
}
...
\iffull\newcommand{\leftrep}{sum\xspace} \newcommand{\rightrep}{product\xspace}
\else  \newcommand{\leftrep}{left\xspace}\newcommand{\rightrep}{right\xspace}
\fi
...
\iffull
In the following explanation, we will break the convention of referring to
the repositories as ``left'' and ``right'', preferring the terms ``sum
repository'' and ``product repository\commaquote to avoid confusion with the
left and right sides of the contained sums and products.
\fi
...
r\'ak (with correct diacritics) in the \leftrep repository. The edit describing
...
\iffull product \else right-hand \fi repository (line \markref{rmodstay} in
...
The \iffull $\dputr$ \else left-to-right \fi translation of other in-place
modifications, insertions, and deletions and the \iffull $\dputl$ \else
right-to-left \fi translation of in-place modifications,
...
Perhaps the most interesting of these is an in-place modification to the
\leftrep repository that switches sides of a sum (line~\markref{rmodswitch}).
%
...
in the interleaved list into a position in the left list in the \rightrep
...
Consider an edit to the \rightrep repository that swaps Schumann and
...
$\mlinl{\mbox{Dvorak}}$ in the \leftrep repository
...
\iffull\footnote{The base cases for $\mlcount$ are unusual, but very
convenient for our purposes. The intuition for $\mlcount(p,c)$ is to imagine
inserting an $L$ (respectively, $R$) before index $p$ in list $c$; then we
output how many $L$s (resp. $R$s) there are up to this newly inserted
item.}\fi. It is convenient to derive an isomorphism between positions in
the \leftrep repository and positions tagged by which list they are indexing
into in the \rightrep repository; the $\mliso$ function shows how to use
$\mlcount$ to do this. In our example, the resulting isomorphism looks like
this:
\begin{center}
\noindent \begin{tabular}{c|ccccc}
    \leftrep  & $1$ & $2$ & $3$ & $4$ & $5$ \\
    \hline
    \rightrep & $\mlinl 1$ & $\mlinl 2$ & $\mlinr 1$ & $\mlinr 2$ & $\mlinl 3$
\end{tabular}
\end{center}
...
where position $p$ in the \leftrep repository should come from, we can simply
translate $p$ into an index into the \rightrep repository using $\mliso$, apply
$g$ to find out where that index came from, and translate back into the \leftrep
...
Translating a reordering of the \leftrep repository follows a similar path (line~\markref{rreorder}):
...
in the \leftrep repository may shuffle which positions are $\mlinlx$'s and which
...
\item {\bf Case \markref{rfail}:} As in Case~\markref{rmodfail}, there is
    ...
    is immediately contradicted.\ifdissertation\endofpf\fi
...
$\dif \in \iffull|X|\times|X|\else X\times X\fi \rightarrow \partial X$
satisfying $\dif(x,x')x=x'$ and $\dif(x,x)=\ONE$. \iffull (The edit
$\dif(x,x')$ is permitted to be partial everywhere except $x$.) \fi
%
