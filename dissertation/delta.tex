...
\end{pf}
% TODO: see if we can use this theorem to simplify some proofs
...
\begin{pf}\ 
    \begin{itemize}
        ...
        \item $\id\oplus\id\equiv\id$
            ...
            the argument for $\dputr$; the argument for $\dputl$ is
            symmetric. Hence we may assume we have consistent triples
            $(x,c,y) \in (\id\oplus\id).K$ and $(x,d,y) \in \id.K$ and an
            edit $\dx$ for which $\dx\;x\Defined$. Since
            $\id.\dputr(\dx,d)=(\dx,\unit)$, we must show
            $(\id\oplus\id).\dputr(\dx,c)=(\dx,c')$ for some $c'$. This
            property is clearly true of $(\id\oplus\id).\dputr\gen$ by
            inspecting the various cases; one need only observe that
            $\dputr\gen$ outputs $\fail$ only in cases where $\dx\;x$ is
            visibly not defined.
        ...
        \item $((k\oplus\ell)\oplus m);\bij_\assoc\equiv
            ...
            \dmwit{proofneeded}\finish{and what is $\bij_\assoc$?}
