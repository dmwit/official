...
G \to M$ on generators, the monoid homomorphism $f \in G\LIST \to M$ is
defined by
$f(\NIL)=\ONE$ and $f(g_1 \cdots g_n)=f_g(g_1)f(g_2 \cdots g_n)$. Similarly, given a
...
$\left<G\LIST,\NIL,\plusplus\right>$ of sequences of elements from $G$
with sequence-append as the binary operation and with the empty sequence as
the identity.
\end{defn}

\begin{defn}[Congruence Closure]
The \emph{congruence closure} $\simeq_R$ of a relation $R \in G\LIST \times
G\LIST$ is the least congruence relation containing $R$: 
...
Given a generator set $G$ and a relation $R \subset G\LIST \times G\LIST$, we can
construct the monoid $G\LIST_R=\left<G\LIST\!\mathord{/}\!\simeq_R, \,
[\NIL]_{\simeq_R}, 
\, [\plusplus]_{\simeq_R}\right>$ of $\simeq_R$-equivalence classes of
$G\LIST$ under concatenation (of representatives). It is
...
We can now \emph{specify} a monoid homomorphism $h \in G\LIST_R \to M$ by giving a
function $h_g \in G \to M$. We extend $h_g$ to $h\liftedtolists \in G\LIST \to M$
in the obvious way: 
\begin{align*}
    h\liftedtolists(\NIL) &= \ONE \\
...
\end{array}
\]
Hence, we can specify a function $h$ of type $G\LIST_R \times C \to \partial Y
...
\in G\LIST_R \to (C \to \partial Y \times C)$, and then using the above
isomorphism to view $h$ as belonging to 
$G\LIST_R \times C \to \partial Y \times C$.
...
$\odot_*$ respects $R$, lifting it to $\odot \in G\LIST_R \to (X \partialto X)$
using Corollary~\ref{cor:lifted}, and using the above
isomorphism\iflater\bcp{ignoring the monoid operator!}\fi to 
view $\odot$ as a function $\odot \in h \in G\LIST_R \times X \partialto X$.
Moreover, the laws for monoid homomorphisms (specialized to the monoids
$G\LIST_R$ and $X \partialto X$) correspond exactly to the monoid action laws.
...
%
On monoids, $\partial X \otimes \partial Y = G\LIST_R$, where \[G = \left\{\mlonl{\dx}
...
%
On monoids, $M \oplus N = G\LIST_R$, where
\[G\ \  = \begin{array}[t]{l@{\ \ }l}
...
\begin{align*}
    ...
    &\mlinb \cond{(n_L+1,n_R) & c_1 = L \\ (n_L,n_R+1) & c_1 = R}
\end{align*}
\else
\begin{align*}
    \mlcount(p,c_1 \cdots c_n)
...
to a module; this time, to the module $\Unit_{L\in\{L,R\}}\LIST$.
...
p<i\}$ and $I'$ comprises prefix closed subsets of $\{0,1\}\LIST$;
...
\else
    ...
    correspondence;
\fi
thus, for example if $i=4$ and $i'=\{\NIL,0,1,11\}$ the bijection
would be as shown above. (For illustration we also indicate possible
$X$-contents of the positions.)
%
Formally, we have $f_{i,c,i'} = \{(0,0), (1,\NIL), (2,1), (3,11)\}$. 
...
\{(0,00)$, $(1,0)$, $(2,01)$, $(3,\NIL)$, $(4,1)$, $(5,11)\}$ and
...
To show that $\dputr$ respects the law $\mlleft\ONE=\NIL$:
\begin{align}
...
$\partial X$ on $X$ is defined by $\NIL x=x$ and $(xw)y=x$
