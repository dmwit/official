...
\item {\bf Case \markref{rdel}:}
    % TODO: Say something about shadowing the conventional $n_L$ and $n_R$
    % bindings. (In fact, in all the other cases, the shadowing is
    % benign because the definitions coincide -- can we say something about
    % that when we set up the convention so we don't have to mention it
    % every time? Then the sentence here will be more memorable because it
    % only appears here, rather than appearing here and merely being
    % different from the other five such sentences.)
    \begin{align*}
        \dz &= \mldel i \\
...
\item {\bf Case \markref{lreorder}:} We have a slew of equalities in hand to
    ...
    \begin{align*}
        ...
        h &= \mliso(c)
    \end{align*}
    Although it may appear as though we are shadowing the earlier
    definitions of $n_L$ and $n_R$, Lemma~\ref{lemma:iso-coherent} tells us
    that the definition here is in agreement with the remainder of this
    proof's established convention.
...
\item {\bf Case \markref{ldeli}:} To fit in with the surrounding
    ...
    \end{align*}
    and we know that $0 < i \le n_L + 1$. Our strategy is to show that
    $(d\;z,c'',(\mldel1\;x,y)) \in K$; the induction hypothesis then
    tells us that $(d'\;(d\;z),c',d''(\mldel1\;x,y))\in K$. This means that
    $((d'd)\;z,c',(\mldel i\;x,y)) \in K$, which concludes this case.

    % TODO: this ``count'' think is super-weird. check out the base cases on
    % that thing! I think we might need to change it... how many places
    % is it used?? (answer: 30 total)

    % Conclusion after some thinking and rereading: do not need to change
    % definition of count, but do need to change all invocations that look
    % like this:
    % (n_L, n_R) = count(|c|, c)
    % to look like this instead:
    % (n_L+1, n_R+1) = count(|c|+1, c)
    % See also the note about conventions in the proof of the rdel case for
    % a good idea about writing that should be implemented at the same time
    % as this change.
...
\iffull \item \else (4) \fi $(\partial_{xy} \ell).\dputr(x\CONS w,c)=(y\CONS v,c'')$ where
...
the induction hypothesis tells us that if
...
$(\partial_{xy}k).\dputr(\dx,c_k)=(y_k\CONS\dy_k,c_k'')$ and
$(\partial_{xy}\ell).\dputr(\dx,c_\ell)=(x_\ell\CONS\dx_\ell,c_\ell'')$. But we
have already seen that $y_k\CONS\dy_k=x_\ell\CONS\dx_\ell$ and $(c_k'',c_\ell'') \in
S$, so we are done.
\end{proof}
\fi
