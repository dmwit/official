              ...
              {k \otimes \ell \in X \otimes Y \lens Z \otimes W}}
{
    ...
    \dputr_g(\mlright\dy,(c_k,c_\ell))
        ...
        & & (\blist\mlright\dw\elist,(c_k,c_\ell')) \\
...
$\blist\mlleft\ONE\elist = \NIL$. See Section~\ref{sec:monoid-laws} for
              ...
              {k \oplus \ell \in X \oplus Z \lens Y \oplus W}}
{
    ...
    \dputr_g(\mlswitchll\dx, \mlinl c)
        &=& \mllet (\dy,c') = k.\dputr(\dx,c_k)
          \ \ml{in}\ (\blist\mlswitchll\dy\elist, \mlinl{c'}) \\
    \dputr_g(\mlswitchrl\dx, \mlinr c)
        &=& \mllet (\dy,c') = k.\dputr(\dx,c_k)
          \ \ml{in}\ (\blist\mlswitchrl\dy\elist, \mlinl{c'}) \\
    \dputr_g(\mlswitchlr\dz, \mlinl c)
        &=& \mllet (\dw,c') = \ell.\dputr(\dz,c_\ell)
          \ \ml{in}\ (\blist\mlswitchlr\dw\elist, \mlinr{c'}) \\
    \dputr_g(\mlswitchrr\dz, \mlinr c)
        &=& \mllet (\dw,c') = \ell.\dputr(\dz,c_\ell)
          \ \ml{in}\ (\blist\mlswitchrr\dw\elist, \mlinr{c'}) \\
    \dputr_g(\mlstayl\dx,\mlinl c)
        &=& \mllet (\dy,c') = k.\dputr(\dx,c)
          \ \ml{in}\ (\blist\mlstayl\dy\elist, \mlinl{c'}) \\
    \dputr_g(\mlstayr\dz,\mlinr c)
        &=& \mllet (\dw,c') = \ell.\dputr(\dz,c)
          \ \ml{in}\ (\blist\mlstayr\dw\elist, \mlinr{c'}) \\
    \iffailed
    \dputr_g(e,\mlfailed )
        &=& \mllet (e',c) = \dputr(e,\missing) \\
        & & \ml{in}\ (e',\mlfailed) \\
    \fi
    \dputr_g(e,c) &=& (\blist\fail\elist,\iffailed \mlfailed \else c \fi)\mbox{ in all other cases} \\[1.2ex]
              ...
              {\ell\LIST \in X\LIST \lens Y\LIST}}
{
    ...
    \dputr_g(\mlmod p\dx,c)
        ...
        & & (\blist\mlmod p\dy\elist,c[p \mapsto c_p'])) \\
        & & \mbox{ when } p \le n \\
    \dputr_g(\mlmod p\dx,c)
        &=& (\blist\fail\elist,c) \mbox{ when } p > n \\
    \dputr_g(\fail,c) &=& (\blist\fail\elist,c) \\
    \dputr_g(\dx,c)
    &=& (\blist\dx\elist,\dx\,c) \mbox{ in all other cases} \\
    \dputl&& \mbox{similar}
}
\makeatletter\refstepcounter{\@captype}\makeatother
    ...
    \begin{itemize}
        ...
        \item Suppose $S$ is a witness that $k \equiv \ell$. Define $S'$ by
            ...
            \infrule
                ...
                {(\blist x_1\cdots x_n\elist,
                  \blist c_1\cdots c_n\elist,
                  \blist d_1\cdots d_n\elist,
                  \blist y_1\cdots y_n\elist)\in S'}
        ...
        \item Define the function $\ml{unzip}$ as follows:
            \[\begin{array}{l}
            \ml{unzip}(\blist(x_1,y_1)\;(x_2,y_2) \cdots (x_n,y_n)\elist) \\
            \qquad = (\blist x_1\;x_2\cdots x_n\elist,\blist y_1\;y_2\cdots y_n\elist)
            \end{array}\]
