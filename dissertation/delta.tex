...
(a), recall from
\S \ref{sec:examples} that, for every set $X$, we can form an {\em
...
\S \ref{sec:examples}, there are situations where the information in an
...
\begin{defn}
Let $\ell \in X\lens Y$ be a lens. A \emph{dialogue} is a sequence of edits---a word in $(\partial X+\partial Y)^*$. The {partial}
function
\iffull\[\else $ \fi
\ell.\run \in (\partial X+\partial Y)^*\partialto   X\times   \ell.C\times Y
\iffull\]\else $ \fi
is defined by:
\infrule{}{\ell.\run(\varepsilon) = (\init_X,\ell.\missing,\init_Y)}
\infrule{
    \ell.\run(w)=(x_0,c,y_0)
    \andalso  
    \ell.\dputr(\dx_1,c)=(\dy_1,c_1)
  }{
    \ell.\run(\mlinl{\dx_1}w) = (\dx_1\,x_0,c_1,\dy_1\,y_0)
  }
\infrule{
    \ell.\run(w)=(x_0,c,y_0)
    \andalso  
    ...
    \ell.\run(\mlinr{\dy_1}w) = (\dx_1\,x_0,c_1,\dy_1\,y_0)
  }
\end{defn}
...
\begin{proof}
    ...
    $\dx\;x$ is defined and $(\dz\;z,(c_k',c_\ell'),\dx;x) \in
    (k\op;\ell\op).K$. So we have reached another consistent quadruple.
\end{proof}
\fi
...
\end{proof}
\begin{lemma} The $\disconnect$ and $\const$ lenses are closely related:
$\disconnect_{XY} \equiv \const_X;\const_Y\op$ and 
$\const_X \equiv \disconnect_{X\Unit}$.
\end{lemma}
\begin{proof}
    The complete relation $\{(\unit,\unit)\}$ is a witness to the
    equivalence $\disconnect_{XY} \equiv \const_X;\const_Y\op$. The latter
    equivalence is actually an equality: $\const_X = \disconnect_{X\Unit}$
    can be verified by inspecting the two definitions.
\end{proof}
\fi

\noindent
The $\disconnect$ lens that we saw in \S \ref{sec:examples} can be
...
Given a set $G$, we write $G^*$ for the set of finite sequences of
elements of $G$. We write $\varepsilon$ for the empty
...
is easy to show that $G^*$ together with sequence concatenation and
$\varepsilon$ forms a monoid.

It is often convenient to specify the behavior of a monoid
homomorphism by giving its output on each generator. Given a function $f_g \in
G \to M$ on generators, the monoid homomorphism $f \in G^* \to M$ is
defined by
$f(\varepsilon)=\ONE$ and $f(g_1 \cdots g_n)=f_g(g_1)f(g_2 \cdots g_n)$. Similarly, given a
stateful function $f_g \in G \times C \to M \times C$, we can define a
stateful monoid homomorphism $f \in G^* \times C \to M \times C$ by setting
$f(\varepsilon,c) = (\ONE,c)$ and
\begin{align*}
...
action for the free monoid $(G^\otimes_{X,Y})^*$:
\begin{align*}
...
\[X \otimes Y =
\left<|X|\times|Y|,(\init_X,\init_Y),(G^\otimes_{X,Y})^*,\odot\right>.\]
...
.
%
\hbox to 0pt{\hspace{1.5em} \endofpfmarker}
\end{array}
\]
\noendofpf
\end{pf}
...
\iffull
This theorem asserts that $\otimes$ is an symmetric, associative
...
generator $\dx$ to $\mlleft \dx$ and extend homomorphically.  In
order that $\dx\mapsto \mlleft\dx$ be a homomorphism of modules in
general we would need the equation $\mlleft \dx \cdot \mlleft{\dx'}=
\mlleft {\dx \, \dx'}$ and similarly for $\ml{right}$. See
Section~\ref{sec:monoid-laws} for more detail on this alteration.  \fi
...
Section~\ref{sec:monoid-laws} we will introduce a more compact
...
side.\discuss{Restored the previous sentence, which referred to ``fail'' not
  ``failing''!}
%
These intuitions are formalized in the application function:
\begin{eqnarray*}
...
\[X \oplus Y =
\left<|X|+|Y|,\mlinl{\init_X},(G^\oplus_{X,Y})^*,\odot\right>.\]
...
\begin{comment}
% This stuff is no longer relevant, as we are including it in other
% sections.
\paragraph*{Monoid presentations}

The above definition of the product of two modules gives the new monoid and
edit application functions directly.  However, it will be more convenient to
define modules for sums and lists using the machinery of {\em monoid
  presentations}. To illustrate this approach and build intuition for the
technicalities, let us begin by re-deriving the above definition for tensor
products using this machinery.

In a monoid presentation, we give a set of \emph{generators} of the monoid
and \emph{equations} over words of generators specifying that certain edits
are to be identified.

\begin{defn}[Free Monoid]
Given a generator set $G$, we can construct the \emph{free monoid}
$\left<G^*,\varepsilon,\plusplus\right>$ of sequences of elements from $G$
with sequence-append as the binary operation and with the empty sequence as
the identity.
\end{defn}

\begin{defn}[Congruence Closure]
The \emph{congruence closure} $\simeq_R$ of a relation $R \in G^* \times
G^*$ is the least congruence relation containing $R$: 
...
\hfil
\begin{minipage}{.32\linewidth}
\infrule{v \simeq_R w}{w \simeq_R v}
\end{minipage} \hfil \\[1ex]
%
\begin{minipage}{.32\linewidth}
\infrule{u \simeq_R v \andalso v \simeq_R w}{u \simeq_R w}
\end{minipage}
\hfil
\begin{minipage}{.37\linewidth}
\infrule{v_1 \simeq_R v_2 \andalso w_1 \simeq_R w_2}
        {v_1 \plusplus w_1 \simeq_R v_2 \plusplus w_2}
\end{minipage}
\hfil
\end{center}
\end{trivlist}
...
\begin{defn}[Monoid Presentation]
Given a generator set $G$ and a relation $R \in G^* \times G^*$, we can
construct the monoid $G^*_R=\left<G^*\!\mathord{/}\!\simeq_R, \,
[\varepsilon]_{\simeq_R}, 
\, [\plusplus]_{\simeq_R}\right>$ of $\simeq_R$-equivalence classes of
$G^*$ under concatenation (of representatives). It is
easy to show that $\plusplus$ respects $\simeq_R$ (and hence that the
operation $[\plusplus]_{\simeq_R}$ is well defined) because $\simeq_R$ is a
congruence relation.  
\end{defn}

We can now \emph{specify} a monoid homomorphism $h \in G^*_R \to M$ by giving a
function $h_g \in G \to M$. We extend $h_g$ to $h\liftedtolists \in G^* \to M$
in the obvious way: 
\begin{align*}
    h\liftedtolists(\varepsilon) &= \ONE \\
    h\liftedtolists(gw) &= h_g(g)\,h\liftedtolists(w)
\end{align*}

\iffull
\begin{lemma}
\label{lem:hstar}
It is easy to see by induction on $v$ that
$h\liftedtolists(v)\,h\liftedtolists(w)=h\liftedtolists(vw)$.
\end{lemma}
\fi

\begin{lemma}
If $h\liftedtolists$ respects $R$---that is, whenever $v \relR w$ we also
have $h\liftedtolists(v)=h\liftedtolists(w)$---then $h\liftedtolists$
respects $\simeq_R$.
\end{lemma}

\iffull
\begin{proof}
By induction on derivations for the
judgment that $v \simeq_R w$. The two most interesting cases are the base
case where $v \relR w$, in which case we conclude that
$h\liftedtolists(v)=h\liftedtolists(w)$ by assumption, and the inductive
case where we factor $v=v_1v_2$ and $w=w_1w_2$. In this case, the induction
hypothesis tells us that $h\liftedtolists(v_1)=h\liftedtolists(w_1)$ and
similarly that $h\liftedtolists(v_2)=h\liftedtolists(w_2)$; by
Lemma~\ref{lem:hstar}, we can now conclude that 
    \begin{align*}
        h\liftedtolists(v)
        &= h\liftedtolists(v_1v_2) \\
        &= h\liftedtolists(v_1)h\liftedtolists(v_2) \\
        &= h\liftedtolists(w_1)h\liftedtolists(w_2) \\
        &= h\liftedtolists(w_1w_2) \\
        &= h\liftedtolists(w),
    \end{align*}
as desired.
\end{proof}
...
\begin{corollary}
\label{cor:lifted}
    If $h\liftedtolists$ respects $R$, then the function
    \[h([w])=h\liftedtolists(w)\]
    is well defined and a monoid homomorphism.
\end{corollary}

In particular, we will use this below when defining the $\dputr$ and
$\dputl$ functions of the tensor product lens. The types 
of these functions have the form $\partial X \times C \to \partial Y
\times C$, which is isomorphic to
$\partial X \to (C \to \partial Y \times C)$. Additionally, $C \to \partial
Y \times C$ is a monoid whenever $\partial Y$ is, with identity $\lambda
c.\; (\ONE,c)$ and binary operation $\cdot$ with
\[(f \cdot g)(c) =
\begin{array}[t]{@{}l}
\mllet (\dy, c') = g(c)
\mlinm \\ \mllet (\dy', c'') = f(c') \mlinm \\ (\dy'\,\dy,c'').
\end{array}
\]
Hence, we can specify a function $h$ of type $G^*_R \times C \to \partial Y
\times C$ by giving a function $h_g \in G \to (C \to \partial Y \times C)$,
extending it to $h_* \in G_* \to (C \to \partial Y \times C)$, checking that
this respects $R$, using Corollary~\ref{cor:lifted} to obtain a function $h
\in G^*_R \to (C \to \partial Y \times C)$, and then using the above
isomorphism to view $h$ as belonging to 
$G^*_R \times C \to \partial Y \times C$.
When giving $h_g$, we will abuse notation slightly, writing
\[h_g(g,c) = (\dy,c')\]
instead of
\[h_g(g) = \lambda c.\; (\dy,c').\]
Since the resulting function $h$ is a monoid homomorphism, it automatically
satisfies the lens laws.\finish{This requires a bit more explanation: the
  lens laws are about stateful monoid homomorphisms, so it may not be
  obvious to readers (it wasn't to me, and I'm still not sure it's
  completely right).}

We can use the same machinery to specify monoid actions by noticing that
$\partial X \times X \partialto X$ is isomorphic to $\partial X \to (X
\partialto X)$ and that $X \partialto X$ is itself a monoid.  So we
can specify a monoid action $\odot$ on $\partial X$ and $X$ by giving a
function $\odot_g \in \partial X \to (X \partialto X)$, showing that
$\odot_*$ respects $R$, lifting it to $\odot \in G^*_R \to (X \partialto X)$
using Corollary~\ref{cor:lifted}, and using the above
isomorphism\iflater\bcp{ignoring the monoid operator!}\fi to 
view $\odot$ as a function $\odot \in h \in G^*_R \times X \partialto X$.
Moreover, the laws for monoid homomorphisms (specialized to the monoids
$G^*_R$ and $X \partialto X$) correspond exactly to the monoid action laws.
For readability, we will write $\odot_g$ in uncurried form---i.e., we will
write
\[g \odot_g x = x'\]
instead of
\[\odot_g(g) = \lambda x.\; x'.\]

With this infrastructure in place, we can define tensor product operators on
sets, monoids, monoid homomorphisms\bcp{actually, we don't define tensor
  product of monoid homomorphisms, but only of stateful ones---i.e.,
  $\dputr/\dputl$ components of lenses.  The terminology of ``stateful
  module homomorphism'' would actually be useful here and in a couple of
  other places.}, modules, and lenses (overloading the
$\otimes$ symbol for all of them).
%
On sets, $X \otimes Y = X \times Y$.
%
On monoids, $\partial X \otimes \partial Y = G^*_R$, where \[G = \left\{\mlonl{\dx}
        \mid \dx \in \partial X\right\} \cup \left\{\mlonr{\dy} \mid \dy \in
        \partial Y\right\}\] and $R$ is defined as follows:
        \begin{align*}
            \mlonl{\ONE} &\eqR  \varepsilon \\
            \mlonr{\ONE} &\eqR  \varepsilon \\
            \mlonl{\dx}\,\mlonl{\dx'} &\eqR  \mlonl{\dx\,\dx'} \\
            \mlonr{\dy}\,\mlonr{\dy'} &\eqR  \mlonr{\dy\,\dy'} \\
            \mlonl{\dx}\,\mlonr{\dy} &\eqR  \mlonr{\dy}\,\mlonl{\dx}
        \end{align*}
%
On the $\dputr/\dputl$ components of lenses, $g \otimes h$ is defined using
Corollary~\ref{cor:lifted} from the function $\otimes_g$ defined as follows: 
        \begin{align*}
            (g \otimes_g h)(\mlonl{\dx},(c_g,c_h)) ={}&
                \mllet (\dx',c') = g(\dx,c_g) \mline \\
            & (\mlonl{\dx'},(c',c_h)) \\
            (g \otimes_g h)(\mlonr{\dy},(c_g,c_h)) ={}&
                \mllet (\dy',c') = h(\dy,c_h) \mline \\
            & (\mlonr{\dy'},(c_g,c')).
        \end{align*}
%
\begin{lemma}
    $g \otimes\liftedtolists h$ respects $R$.
\end{lemma}
%
On modules, \[X \otimes Y = \left<|X| \otimes
        |Y|,(\init_X,\init_Y),\partial X \otimes \partial Y,\odot\right>\]
        where $\odot$ is specified (using the 
        notation suggested above for monoid action specifications) by
        \begin{align*}
            \mlonl{\dx} \odot_g (x,y) &= (\dx\, x,y) \\
            \mlonr{\dy} \odot_g (x,y) &= (x,\dy\, y).
        \end{align*}
%
\begin{lemma}
    $\odot\liftedtolists$ respects $R$.
\end{lemma}
%
And finally, for lenses:

\begin{defn}[Tensor Product]\ \label{w_product}
\lensdef{w_product}
{\infruleplain
    {k \in X \lens Y \andalso \ell \in Z \lens W}
    {k \otimes \ell \in X \otimes Z \lens Y \otimes W}
}{
    C &=& k.C \otimes \ell.C \\
    \missing &=& (k.\missing, \ell.\missing) \\
    \dputr &=& k.\dputr \otimes \ell.\dputr \\
    \dputl &=& k.\dputl \otimes \ell.\dputl \\
    K &=& \{ \; ((x,z),(c_k,c_\ell),(y,w)) \mid \\
           && \qquad \hspace{.7em} \, (x,c_k,y) \in k.K \\
           && \qquad \wedge \,
               (z,c_\ell,w) \in \ell.K \; \}
...
It is not hard to show that the above construction, while rather more
verbose, is essentially identical to the original one.\footnote{This is not
  the only sensible definition of the tensor product. For example, one might
  discover a situation in which a lens might need to inspect the order in
  which the two elements of a tuple were edited (at the cost of increasing
  the bandwidth required to communicate the edit history). Such a product
  arises by omitting the law $\mlonl\dx\mlonr\dy=\mlonr\dy\mlonl\dx$; still
  other tensor products arise by omitting other laws. Nevertheless, the
  definition of the tensor product \emph{lens} can remain the
  same.}
\iflater
\dmwit{Should probably write down the lemmas/theorems we need to
  make this precise.}\dmwit{I've proven the necessary things with
  a bad notation, but it's too late at night just now to do a good job of
  picking a good notation. Anyway, it's not a conjecture that the two
  definitions result in the same thing.} 
\fi
The first presentation, where we directly defined
$\partial(X \otimes Y) = \partial X \times \partial Y$, amounts to finding a
\emph{canonical form} for words of generator edits; in the second
presentation we first define the monoid, then separately prove that there
are canonical forms.
\fi

\iflater

As we did for the presentation of tensor products, we can define the summing
operation on sets, monoids, monoid homomorphisms, modules, and lenses
(writing $\oplus$ for all of them).
%
On sets, $X \oplus Y = X + Y$ is the disjoint sum.
%
On monoids, $M \oplus N = G^*_R$, where
\[G\ \  = \begin{array}[t]{l@{\ \ }l}
              & \{\mlswitchl,\mlswitchr,\mltoggle\} \\
              \cup& \{\mlstayl m \mid m \in M\} \\
              \cup& \{\mlstayr n \mid n \in N\}
\end{array}\]
and $R$ is given by: 
\begin{align*}
            \mlswitchl\,g &\eqR  \mlswitchl \\
            \mlswitchr\,g &\eqR  \mlswitchr \\
            \mltoggle\,\mlswitchl &\eqR  \mlswitchr \\
            \mltoggle\,\mlswitchr &\eqR  \mlswitchl \\
            \mltoggle\,\mlstayl m &\eqR  \mltoggle \\
            \mltoggle\,\mlstayr n &\eqR  \mltoggle \\
            \mltoggle\,\mltoggle\,\mltoggle &\eqR  \mltoggle \\
            \mlstayl \ONE &\eqR \varepsilon \\
            \mlstayr \ONE &\eqR \varepsilon \\
            \mlstayl m\,\mlstayl{m'} &\eqR  \mlstayl{m\,m'} \\
            \mlstayr n\,\mlstayr{n'} &\eqR  \mlstayr{n\,n'} \\
            \mlstayl m\,\mlstayr n &\eqR  \mlstayr n\,\mlstayl m
\end{align*}
%
For $\dputr$/$\dputl$, we parameterize the $\oplus$ operator by the
$\missing$ complements of the underlying lenses (so that edits like
$\mlswitchl$ that need to reset the complement can):
\discuss{The rest of the definition has changed to be consistent with the
  new form of $\missing$ for sum lenses.  But I'm not confident yet that
  these rules are right (note the ``error cases'' for stay).}\dmwit{Yeah, it
  looks like this proof will have to be completely redone.}
\begin{align*}
   (g\oplus^{(c_g,c_h)}_gh)(\mlswitchl,\_) ={}& (\mlswitchl,\mlinl{c_g}) \\
   (g\oplus^{(c_g,c_h)}_gh)(\mlswitchr,\_) ={}& (\mlswitchr,\mlinr{c_h}) \\
   (g\oplus^{(c_g,c_h)}_gh)(\mltoggle ,\mlinl{c}) ={}& (\mltoggle ,\mlinr{c_h}) \\
   (g\oplus^{(c_g,c_h)}_gh)(\mltoggle ,\mlinr{c}) ={}& (\mltoggle ,\mlinl{c_g}) \\
   (g\oplus^c_gh)(\mlstayl m,\mlinl{c_g}) ={}& \mllet (m',c_g') = g(m,c_g) \mline \\
                                         & (\mlstayl{m'},\mlinl{c_g'}) \\
   (g\oplus^c_gh)(\mlstayl m,\mlinr{c_h}) ={}& \mllet (m',c_g') = g(m,k.\missing) \mline \\
                                         & (\mlstayl{m'},\mlinr{c_h}) \\
   (g\oplus^c_gh)(\mlstayr n,\mlinl{c_g}) ={}& \mllet (n',c_h') = h(n,\ell.\missing) \mline \\
                                         & (\mlstayr{n'},\mlinl{c_g}) \\
   (g\oplus^c_gh)(\mlstayr n,\mlinr{c_h}) ={}& \mllet (n',c_h') = h(n,c_h) \mline \\
                                         & (\mlstayr{n'},\mlinr{c_h'})
\end{align*}
%
\begin{lemma}
    $g\oplus^c\liftedtolists h$ respects $R$.
\end{lemma}
%
On modules, we (arbitrarily) choose to left-bias the dummy
        elements:
        \[X \oplus Y = \left<|X|\oplus|Y|,\mlinl{\init_X},\partial
        X\oplus\partial Y,\odot\right>\]
        We specify the monoid action $\odot$: 
        \begin{align*}
            \mlswitchl \odot_g v &= \mlinl{\init_X} \\
            \mlswitchr \odot_g v &= \mlinr{\init_Y} \\
            \mltoggle \odot_g \mlinl x &= \mlinr{\init_Y} \\
            \mltoggle \odot_g \mlinr y &= \mlinl{\init_X} \\
            \mlstayl \dx \odot_g \mlinl x &= \mlinl{\dx\,x} \\
            \mlstayl \dx \odot_g \mlinr y &= \mlinr y \\
            \mlstayr \dy \odot_g \mlinl x &= \mlinl x \\
            \mlstayr \dy \odot_g \mlinr y &= \mlinr{\dy\,y}
        \end{align*}
%
\begin{lemma}
              ...
              {k \oplus \ell \in X \oplus Z \lens Y \oplus W}}
{
    ...
    \dputr &=& k.\dputr \oplus^\missing \ell.\dputr \\
    \dputl &=& k.\dputl \oplus^\missing \ell.\dputl \\
    K &=& \hspace{.85em} \{ \; (\mlinl{x},\mlinl{c_k},\mlinl{y}) \mid (x,c_k,y) \in k.K \} \\
      && \cup \,
         \{ \; (\mlinr{z},\mlinr{c_\ell},\mlinr{w}) \mid (z,c_\ell,w) \in \ell.K \} \\
    }
\end{defn}

\PENDING{conjecture: this has many of the properties we expect from a
monoidal bifunctor}

There is a convenient canonical form for edits of the sum module. First, we
define a module of ``structural'' changes to sums; given modules $X$ and
$Y$, we define:
\begin{align*}
    \Tag &= \{\ml{inl},\ml{inr}\} \\
    \TagM &= \left<\Unit+(\Tag\to\Tag),\cdot,\mlinl\unit\right> \\
    \TagP &= \left<|X|+|Y|,\mlinl{\init_X},\TagM,\odot\right> \\
    \ONE \cdot m &= m \\
    m \cdot \ONE &= m \\
    \mlinr f \cdot \mlinr g &= \mlinr{g;f} \\
    \ONE \odot v &= v \\
    \mlinr f \odot \mlinl x &= \left\{\begin{array}{ll}
        \mlinl{\init_X} & f(\ml{inl}) = \ml{inl} \\
        \mlinr{\init_Y} & f(\ml{inl}) = \ml{inr}
        \end{array}\right. \\
    \mlinr f \odot \mlinr y &= \left\{\begin{array}{ll}
        \mlinl{\init_X} & f(\ml{inr}) = \ml{inl} \\
        \mlinr{\init_Y} & f(\ml{inr}) = \ml{inr}
        \end{array}\right. \\
\end{align*}

Then the canonical forms of the above construction consist of a structural
change followed by a pair of edits from the underlying modules (only one of
which will be applied).

\begin{align*}
    M \oplus N &= \left<M \times N \times \TagM,\cdot,(\ONE_M,\ONE_N,\ONE_\TagM)\right> \\
    X \oplus Y &= \left<|X|+|Y|,\mlinl{\init_X},\partial X\oplus\partial Y,\odot\right>
\end{align*}
\begin{align*}
    (m,n,\ONE)\cdot(m',n',p) &= (mm',nn',p) \\
    (m,n,\mlinr f)\cdot(m',n',p) &= (m,n,\mlinr fp) \\
    (m,n,p) \odot v &= \left\{\begin{array}{ll}
        \mlinl{mx} & pv = \mlinl x \\
        \mlinr{ny} & pv = \mlinr y
        \end{array}\right.
\end{align*}
...
\begin{theorem}
    This definition of the sum module is isomorphic to the other one.
\end{theorem}
...
from what we saw in \S \ref{sec:examples}.  
...
\ell.C}^*$ module to define the new complement.
...
\begin{lemma}
    ...
    \begin{itemize}
        \item If $\ell$ is a lens, then $\ell^*$ is a lens.
        \item If $k\equiv\ell$ then $k^*\equiv\ell^*$.
        \item $\id^*\equiv\id$
        \item $k^*;\ell^*\equiv(k;\ell)^*$
    \end{itemize}
...
\begin{proof}\ 
    ...
    \begin{itemize}
        \item Because the lens is defined by specification over a free
            ...
            \map(\ell).\missing = \init_Y = \varepsilon$, it is clear that
        ...
        \item Suppose $S$ is a witness that $k \equiv \ell$. Define $S'$ by
            ...
            \infrule
                ...
                {(x_1\cdots x_n,c_1\cdots c_n,d_1\cdots d_n,y_1\cdots y_n)\in S}
            Then $S'$ is a witness that $k^*\equiv\ell^*$. The initial
            quadruple
            $(\init_{X^*},k^*.\init,\ell^*.\init,\init_{Y^*})=(\varepsilon,\varepsilon,\varepsilon,\varepsilon)$
        ...
        \item Let $f$ be the function that takes a list and returns a list
            ...
            witnesses the equivalence $\id_X^* \equiv \id_X$.
        \item Define the function $\ml{unzip}$ as follows:
            \[\begin{array}{l}
            \ml{unzip}((x_1,y_1)\;(x_2,y_2) \cdots (x_n,y_n)) \\
            \qquad = (x_1\;x_2\cdots x_n,y_1\;y_2\cdots y_n)
            \end{array}\]
            Then the requisite simulation relation is:
            \[R = \{(x,\ml{unzip}(d),d,y) \mid (x,d,y) \in (k;\ell)^*.K\}\]

            The interesting property to verify is that if $(x,c,d,y) \in R$
            and $\dx x$ is defined, then $(k^*;\ell^*).\dputr$ and
            $(k;\ell)^*.\dputr$ produce the same edit $\dy$ in related
            ...
            and both $k^*;\ell^*$ and $(k;\ell)^*$ will merely apply the
...
\begin{figure*}[t!]
\lensdef{partition}
{\partition \in (X \oplus Y)^* \lens X^* \otimes Y^*}
{
    C &=& \{L,R\}^* \\
    \missing &=& \varepsilon \\
    K &=& 
       \{(z, \ml{map}_\ml{tagof}(z), (\ml{lefts}(z),\ml{rights}(z)))
    \mid z \in (|X|+|Y|)^*\}
 \\[1.5ex]
    \dputr_g(\mlmod p\dv,c) &=& (\fail,c)\mbox{ when }p > |c|
    \markeqn{rmodbigp}%
 \\
    \dputr_g(\mlmod p\varepsilon,c) &=& (\varepsilon,c)\mbox{ when }1 \le p \le |c|
    \markeqn{rmodempty}%
 \\
    \dputr_g(\mlmod p{\dv\dv s},c)
        &=& (d'\,d,c'') \WHERE
      \begin{array}[t]{@{}lll}
            1 < n
            &
 ...
 \\
    \dputr_g(\mlmod p{\mlswitch_{jk}(\dv)},c)
        &=& \partitionarray{(d_2d_1d_0,c[p \mapsto k])}{
            (p_L,p_R) &=& \mlcount(p,c) &
            d_0 &=& \ml{map}_{\lambda d.\,\mltag(j,d)}(\mldelete'(p_j)) \\
            d_2 &=& \mltag(k,\mlmod{p_k}\dv) &
            d_1 &=& \ml{map}_{\lambda d.\,\mltag(k,d)}(\mlinsert'(p_k))
        }
        {rmodswitch}%
 \\
...
\\[5ex]
    \dputl_g(\mlonl{\mlmod p\dx},c)
        &=& \partitionarray{(\mlstayl{\mlmod{p'}\dx},c)}{
            p' &=& \mliso(c)^{-1}(\mlinl p)
        }
        {lmod}\\
    ...
    \dputl_g(\mlonl{\mlins i},c) &=& (\mlins i,\mlins i\;c)
      \markeqn{lins}
\\
    \dputl_g(\mlonl{\mldel 0},c) &=& (\varepsilon,c)
      \markeqn{ldel0}
\\
    \dputl_g(\mlonl{\mldel i},c)
        ...
        {ldeli} \\
            && % \hspace{6.7em} 
            \mbox{when }0 < i \le n_L\mathord{+1}
\\
    ...
    \begin{center}
        \begin{minipage}[t]{0.25\linewidth}
            \begin{align*}
                ...
                \ml{lefts}(\varepsilon) &= \varepsilon \\
                \ml{lefts}(\mlinl xw) &= x\,\ml{lefts}(w) \\
                \ml{lefts}(\mlinr yw) &= \ml{lefts}(w) \\[1ex]
                \mltag(L,\dx) &= \mlonl\dx \\
                \mltag(R,\dy) &= \mlonr\dy \\[1ex]
                \mlcount(p,\varepsilon) &= (1,1)
            \end{align*}%
        \end{minipage}%
        \hfill
        \begin{minipage}[t]{0.1\linewidth}
            \begin{align*}
                \ml{map}_f(\varepsilon) &= \varepsilon \\
                \ml{map}_f(cw) &= f(c)\,\ml{map}_f(w) \\[1ex]
                \ml{rights}(\varepsilon) &= \varepsilon \\
                \ml{rights}(\mlinl xw) &= \ml{rights}(w) \\
                \ml{rights}(\mlinr yw) &= y\,\ml{rights}(w) \\[1ex]
                \mluntag(\mlinl x) &= x \\
...
\begin{minipage}[t]{.45\linewidth}
    \begin{align*}
        ...
        \mlreverse(c_1 \cdots c_n) &= c_nc_{n-1}\cdots c_1 \\
        \mldelete'(p) &= \mldel1\,\mlreorder{\ml{cycle}_p} \\
        \mlinsert'(p) &= \mlreorder{\lambda n.\;\ml{cycle}_p(n)^{-1}}\,\mlins1 \\
        \mliso(c) &=
...
Figures~\ref{fig:definition-partition}
and~\ref{fig:definition-partition-supplementary} give the definition of a
list partitioning lens that (as we saw in
\S \ref{sec:examples}) separates a list of tagged elements into
those tagged $\ml{inl}$ and those tagged $\ml{inr}$. 
%
We write $\fail$ to stand for $\mlleft\fail\mlright\fail$ when defining
$\dputr_g$.
%
Additionally, as with the mapping lens, we consider the complement to belong
to a module; this time, to the module $\Unit_{L\in\{L,R\}}^*$.
...
this has the form 
$\mlmod5{\mlstayl{\d n}}$, where $\d n$ describes the string edit to the
...
Figure~\ref{fig:definition-partition}).  We can do this by simply counting
how many composers 
...
$g(\mlinl p)=\mlinl{f(3)(p)}$ and $g(\mlinr p)=\mlinr p$. Then, to find out
...
\begin{tabular}{@{}c|ccccc@{}}
    left  & $1$ & $2$ & $3$ & $4$ & $5$ \\
    $\mliso(\mbox{left})$ & $\mlinl 1$ & $\mlinl 2$ & $\mlinr 1$ & $\mlinr 2$ & $\mlinl 3$ \\
    $g(\mliso(\mbox{left}))$ & $\mlinl 3$ & $\mlinl 2$ & $\mlinr 1$ & $\mlinr 2$ & $\mlinl 1$ \\
    $\mliso^{-1}(g(\mliso(\mbox{left})))$ & $5$ & $2$ & $3$ & $4$ & $1$
\end{tabular}
\end{center}
...
\begin{lemma}
    ...
    \[\mlmod p{\dv\dv s}z=\mlmod p\dv\mlmod p{\dv s}z\]
\end{lemma}
...
\begin{proof}
    ...
    \begin{align*}
        \mlmod p{\dv\dv s}z
            &= z_1\cdots z_{p-1}((\dv\dv s)z_p)z_{p+1}\cdots z_n \\
            &= z_1\cdots z_{p-1}(\dv(\dv sz_p))z_{p+1}\cdots z_n \\
            &= \mlmod p\dv(z_1\cdots z_{p-1}(\dv sz_p)z_{p+1}\cdots z_n) \\
...
If $1 \le p \le n$, then:
\[\mldelete'(p) \odot v_1 \cdots v_n = v_1 \cdots v_{p-1} v_{p+1} \cdots v_n\]
\end{lemma}

\begin{proof}
The only tricky part of this proof is evaluating $\ml{cycle}_p(n)$:
\begin{align*}
    \mldelete'(p) \odot v_1\cdots v_n
        &= \mldel 1\mlreorder{\ml{cycle}_p} \odot v_1\cdots v_n \\
        &= \mldel 1 \odot v_{\ml{cycle}_p(n)(1)} \cdots v_{\ml{cycle}_p(n)(n)} \\
        &= \mldel 1 \odot v_1 \cdots v_{p-1} v_{p+1} \cdots v_{n-1} v_p \\
        &= v_1 \cdots v_{p-1} v_{p+1} \cdots v_{n-1}
\end{align*}
...
evaluation given in these equations a special case where the $v_{p+1}\cdots
v_{n-1}$ interval is empty and $v_p=v_n$. On the other hand, when $p<n$, the
evaluation of $\ml{cycle}_p(n)$ is exactly in the form given here.
\end{proof}

\begin{lemma}
\label{lemma:high-level-insert}
When $1 \le p \le n+1$:
\[\mlinsert'(p) \odot v_1 \cdots v_n = v_1 \cdots v_{p-1} \init\,v_p \cdots v_n\]
\end{lemma}

\begin{proof} Set $v_{n+1}=\init$ so that:
\begin{align*}
    \mlinsert'(p) \odot v_1 \cdots v_n
        &= \mlreorder{\lambda n.\ \ml{cycle}_p(n)^{-1}}\mlins 1 \odot v_1 \cdots v_n \\
        &= \mlreorder{\lambda n.\ \ml{cycle}_p(n)^{-1}} \odot v_1 \cdots v_n\init \\
        &= \mlreorder{\lambda n.\ \ml{cycle}_p(n)^{-1}} \odot v_1 \cdots v_{n+1} \\
        &= v_{\ml{cycle}_p(n+1)^{-1}(1)} \cdots v_{\ml{cycle}_p(n+1)^{-1}(n+1)} \\
        &= v_1 \cdots v_{p-1} v_{n+1} v_p \cdots v_n \\
        &= v_1 \cdots v_{p-1} \init\,v_p \cdots v_n
\end{align*}
...
the base case, $v=\varepsilon$, and:
\begin{align*}
    \ml{lefts}(vw)
        &= \ml{lefts}(w) \\
        &= \varepsilon\ml{lefts}(w) \\
        &= \ml{lefts}(\varepsilon)\ml{lefts}(w) \\
        &= \ml{lefts}(v)\ml{lefts}(w)
\end{align*}
Otherwise, $v=v_1v'$, we know from the induction hypothesis that
...
\begin{align*}
    \ml{lefts}(vw)
        &= \ml{lefts}(\mlinl xv'w) \\
        &= x\ml{lefts}(v'w) \\
        &= x\ml{lefts}(v')\ml{lefts}(w) \\
        &= \ml{lefts}(\mlinl xv')\ml{lefts}(w) \\
        &= \ml{lefts}(v)\ml{lefts}(w)
\end{align*}
In the latter:
\begin{align*}
...
Choose arbitrary $v\in(X+Y)\LIST$ and let $c=\ml{map}_{\ml{tagof}}(v)$ be
...
\begin{proof}
Suppose there are $n_L$ copies of $L$ and $n_R$ copies of $R$ in $c_1 \cdots
c_{p-1}$ and $p \le n+1$. Then it is easy to show (by induction on $c$) that
$\ml{count}(p,c_1 \cdots c_n)=(1+n_L,1+n_R)$. Inspecting the definition of
$\mliso$, it is therefore clear that $\mliso(c)(p)=\mlinl{p'}$ exactly when
$c_p=L$ and there are $p'$ copies of $L$ in $c_1 \cdots c_p$. This implies
there are exactly $p'$ elements tagged $\ml{inl}$ in $v_1 \cdots v_p$ (and
that $v_p$ itself is tagged $\ml{inl}$), hence that
...
\begin{theorem}
The $\partition$ defined in Figures~\ref{fig:definition-partition}
and~\ref{fig:definition-partition-supplementary} is indeed a symmetric edit
lens.   
...
this is immediate.  Second, the initial state \[(\init_{(X \oplus
  Y)^*}, \varepsilon, \init_{X^* \otimes Y^*})\] must be an element of $K$;
...
First we show that $\dputr_g$ respects $K$. We are given some $(z,c,(x,y))
\in K$ and some $\dz \in G^{\mathrm{list}}_{X \oplus Y}$ such that $\dz\;z$
is defined. By the definition of $K$, we know
\begin{align*}
    c &= \ml{map}_{\ml{tagof}}(z) \\
    x &= \ml{lefts}(z) \\
    y &= \ml{rights}(z).
\end{align*}
We can define $(\dz',c')=\dputr_g(\dz,c)$; then we must show
that $\dz'(x,y)$ is defined and that $(\dz\;z,c',\dz'(x,y)) \in
K$. We proceed by induction on the size of $\dz$. We will define $n=|z|$,
$n_L=|x|$, and $n_R=|y|$ in the following.
\begin{trivlist}
...
\item {\bf Case \markref{rmodempty}:}
    $\dz=\mlmod p\varepsilon$ and $1 \le p \le |c|$.
...
\item {\bf Case \markref{rmodcons}:}
    ...
    p{\dv\dv s}z$ is defined, we know $\mlmod p\dv\mlmod p{\dv s}z$ is
...
\item {\bf Case \markref{rmodswitch}:}
    ...
    \begin{align*}
        ...
        \dz' &= d_2d_1d_0 \\
        c' &= c[p \mapsto k] \\
        d_0 &= \ml{map}_{\lambda d.\,\ml{tag}(j,d)}(\mldelete'(p_j)) \\
        d_1 &= \ml{map}_{\lambda d.\,\ml{tag}(k,d)}(\mlinsert'(p_k)) \\
    ...
    \begin{align*}
        \ml{map}_{\ml{tagof}}(\dz\;z)
            &= \ml{map}_{\ml{tagof}}(z_1\cdots z_{p-1}\mlinl{v'}z_{p+1}\cdots z_n) \\
            &= \ml{map}_{\ml{tagof}}(z_1\cdots z_{p-1})\;k\;\ml{map}_{\ml{tagof}}(z_{p+1}\cdots z_n)\\
            &= c_1\cdots c_{p-1}kc_{p+1}\cdots c_n \\
            &= c[p \mapsto k] \\
            &= c'
    \end{align*}
    The second line follows from the first because $\ml{map}$ is a list
    ...
    \begin{align*}
        \dz'\;(x,y)
            &= d_2d_1d_0(x,y) \\
            &= d_2d_1(\ml{map}_{\lambda d.\ml{tag}(j,d)}(\mldelete'(p_j)))(x,y) \\
            &= d_2d_1(\ml{map}_{\ml{left}}(\mldelete'(p_L)))(x,y) \\
            &= d_2d_1(\mldelete'(p_L)x,y) \\
            &= d_2(\ml{map}_{\lambda d.\ml{tag}(k,d)}(\mlinsert'(p_k)))(\mldelete'(p_L)x,y) \\
            &= d_2(\ml{map}_{\ml{left}}(\mlinsert'(p_L)))(\mldelete'(p_L)x,y) \\
    ...
    \begin{align*}
        \dz'\;(x,y)
            &= (\mlmod{p_L}\dv\mlinsert'(p_L)\mldelete'(p_L)(x_1\cdots x_{n_L}),y) \\
            &= (\mlmod{p_L}\dv\mlinsert'(p_L)(x_1\cdots x_{p_L-1}x_{p_L+1}\cdots x_{n_L}),y) \\
            &= (\mlmod{p_L}\dv(x_1\cdots x_{p_L-1}\init_Xx_{p_L+1}\cdots x_{n_L}),y) \\
            &= (x_1\cdots x_{p_L-1}v'x_{p_L+1}\cdots x_{n_L},y)
    \end{align*}
    We now make some observations about the effects of $\dz$, making crucial
    use of Lemma~\ref{lemma:lefts-rights-homomorphism}:
    \begin{align*}
        \ml{rights}(\dz\;z)
            &= \ml{rights}(z_1 \cdots z_{p-1} \mlinl{v'} z_{p+1} \cdots z_n) \\
            &= \ml{rights}(z_1 \cdots z_{p-1})\ml{rights}(\mlinl{v'})\ml{rights}(z_{p+1} \cdots z_n) \\
            &= \ml{rights}(z_1 \cdots z_{p-1})\ml{rights}(\mlinl{v})\ml{rights}(z_{p+1} \cdots z_n) \\
            &= \ml{rights}(z_1 \cdots z_{p-1})\ml{rights}(z_p)\ml{rights}(z_{p+1} \cdots z_n) \\
            &= \ml{rights}(z_1 \cdots z_{p-1}z_pz_{p+1} \cdots z_n) \\
            &= \ml{rights}(z) \\
            &= y
    \end{align*}
    We now observe that Lemma~\ref{lemma:iso-coherent} implies that
    $\ml{lefts}(z_1 \cdots z_{p-1})=x_1 \cdots x_{p_L-1}$ and likewise that
    $\ml{lefts}(z_{p+1} \cdots z_n)=x_{p_L+1} \cdots x_{n_L}$.
    \begin{align*}
        \ml{rights}(\dz\;z)
            &= \ml{lefts}(z_1 \cdots z_{p-1} \mlinl{v'} z_{p+1} \cdots z_n) \\
            &= \ml{lefts}(z_1 \cdots z_{p-1})\ml{lefts}(\mlinl{v'})\ml{lefts}(z_{p+1} \cdots z_n) \\
            &= x_1\cdots x_{p_L-1}v'x_{p_L+1}\cdots x_{n_L}
    \end{align*}
...
\item {\bf Case \markref{rmodstay}:} Let us consider specifically the case
    ...
    any non-$\ml{inl}$ values, so $\ml{map}_{\ml{tagof}}(\dz\;z) =
    \ml{map}_{\ml{tagof}}(z) = c$ and $\ml{rights}(\dz\;z) = \ml{rights}(z)
    = y$. Furthermore:
    \begin{align*}
        \ml{lefts}(\dz\;z)
        &= \ml{lefts}(\mlmod p{\mlstayl\dv}\;z_1 \cdots z_{p-1} \mlinl v
        z_{p+1} \cdots z_n) \\
        &= \ml{lefts}(z_1 \cdots z_{p-1} \mlinl{\dv\;v} z_{p+1} \cdots z_n) \\
        &= x_1 \cdots x_{p_L-1} (\dv\;x_{p_L}) x_{p_L+1} \cdots x_n \\
        &= \mlmod{p_L}{\dv}\;x
    \end{align*}
...
\\
        c' &= c\replicate iL
    \end{align*}
    Now, since $\ml{map}$ is a list homomorphism, we have:
    \begin{align*}
        \ml{map}_\ml{tagof}(\dz\;z)
            &= \ml{map}_\ml{tagof}(z)\ml{map}_\ml{tagof}\left(\replicate i{\mlinl{\init_X}}\right) \\
            &= c\replicate iL \\
            &= c'
    \end{align*}
    ...
    $\dz'(x,y)=(\ml{lefts}(\dz\;z),\ml{right}(\dz\;z))$ which, together with
    the observation above that $\ml{map}_\ml{tagof}(\dz\;z)=c'$, shows that
...
\item {\bf Case \markref{rdel}:}
    ...
    \begin{align*}
        ...
        &= \mldel1\ml{lefts}(z_1 \cdots z_{n-i}\mlinl x) \\
        &= \mldel1(\ml{lefts}(z_1 \cdots z_{n-i})x) \\
        &= \ml{lefts}(z_1 \cdots z_{n-i}) \\
        &= \ml{lefts}(\mldel i\;z)
    \end{align*}
    ...
    \begin{align*}
        ...
        &= \ml{lefts}(z_1 \cdots z_{n-i}\mlinr y) \\
        &= \ml{lefts}(z_1 \cdots z_{n-i}) \\
        &= \ml{lefts}(\mldel i\;z)
    \end{align*}
    as desired.
...
\item {\bf Case \markref{rreorder}:} Before we begin, we address the two
    apparently conflicting definitions of $n_L$ and $n_R$: one from the
    convention established at the beginning of this proof and one from the
    local definitions for this specific case. However,
    Lemma~\ref{lemma:iso-coherent} tells us that these are in fact
    ...
    \begin{align}
        \ml{map}_{\ml{tagof}}(\dz\;z)
            &= \ml{map}_{\ml{tagof}}(z_{f(n)(1)}\cdots z_{f(n)(n)})
            \label{eq:reorder-z} \\
            &= \ml{map}_{\ml{tagof}}(z)_{f(n)(1)}\cdots\ml{map}_{\ml{tagof}}(z)_{f(n)(n)}
            \label{eq:float-index} \\
            &= c_{f(n)(1)} \cdots c_{f(n)(n)}
            \label{eq:def-c} \\
            &= \mlreorder f\;c
    ...
    is a special property of $\ml{map}$; equation~\ref{eq:def-c} by
...
We now show that $\dputl_g$ respects $K$. We will give the proofs for atomic
edits of the form $\mlonl\dx$; the proofs for edits of the form $\mlonr\dy$
are similar. We take some $(z,c,(x,y)) \in K$ and define $n,n_L,n_R$ as
before. Choose $\dx \in G^{\mathrm{list}}_X$ such that $\dx\;x$ is defined.
We define $(\dz,c')=\dputl_g(\mlonl\dx,c)$ and must show that $\dz\;z$ is
defined and that $(\dz\;z,c',(\dx\;x,y)) \in K$. We proceed by induction on
the size of $\dx$.

\begin{trivlist}

\item {\bf Case \markref{lmod}:}\dmwit{proofneeded}

\item {\bf Case \markref{lreorder}:}\dmwit{proofneeded}

\item {\bf Case \markref{lins}:} We compute:
    \begin{align*}
        \ml{map}_{\ml{tagof}}(\dz\;z)
            &= \ml{map}_{\ml{tagof}}(z \replicate i{\init_{X \oplus Y}}) \\
            &= \ml{map}_{\ml{tagof}}(z)\ml{map}_{\ml{tagof}}(\replicate i{\init_{X \oplus Y}}) \\
            &= c\replicate iL \\
            &= \mlinsert(i)\;c \\
            &= c'
    \end{align*}
    ...
    shown that $(\dz\;z,(\dx\;x,c',y)) \in K$.

\item {\bf Case \markref{ldel0}:} Since $\dz\;z=z$, $\dx\;x=x$, and $c'=c$,
    we are in the happy situation of having assumed exactly what we need to
    prove, namely that $(\dz\;z,c',(\dx\;x,y))=(z,c,(x,y)) \in K$.

\item {\bf Case \markref{ldeli}:}\dmwit{proofneeded}

\item {\bf Case \markref{ldelfail}:} When $i>n_L+1$ we find that $\dx\;x$ is
    not defined, contradicting our assumption that it is.

\item {\bf Case \markref{lfail}:} The assumption that $\dx$ successfully
    applies is immediately contradicted, so there is nothing to prove in
    this case.
...
whose shapes are prefix-closed subsets of $\{0,1\}^*$ (access paths)
...
\cite{HofmannPierceWagner10}, we require a \emph{partially-ordered}
...
if, whenever $\di\ i$ is defined, $f(i)$ restricted to $\live(i)$ is a
bijection to $\live(\di\,i)$.
\end{defn}
...
\label{def:containereditapplication}
    ...
    \begin{tabular}{l}
        ...
        $\mldel\di \ (i,f) = (\di\ i,f{\upharpoonright}\live(\di\ i))$ \\
...
%
The $\mlinsert(\di)$ edit alters the shape by $\di$, growing the set of
positions in the process (since $\di\ i\geq i$).  The new positions are
filled with $\init_X$.
%
The $\mldelete(\di)$ edit works similarly, but the set of positions may
shrink; the contents of deleted positions are discarded.
%
...
the basic edits defined above. In Section~\ref{sec:monoid-laws} we discuss
...
\begin{example}
  For any module $X$ we can construe the list module $X^*$ as a
...
\iffull \begin{defn}\ \else \begin{figure} \fi
\lensdef{w_container}
...
%
  ...
  An $\mlmod p\dx$ edit is transported by sending $\dx$ through
  ...
  (i,f)$ succeeds, then $p\in\live(i)$, hence $p\in\live(j)$ and
  $p\in\live(i')$. So the
  result of propagating $\mlmod p\dx$ will be $\mlmod p\dy$
  where $(\dy,c')=\ell.\dputr(\dx,g(p))$. Now since
  $(f(p),g(p),f'(p))\in\ell.K$, we know that $\dy\ f'(p)$ succeeds and
  $(\dx\ f(p), c', \dy\ f'(p))\in\ell.K$. It follows that
  $\mlmod p\dy\ (i',f')$ succeeds and the new triple is again in
  $K$.
...
% \mxh{If $\ell$ has a nontrivial complement then the mapping lens needs to store i in addition. If, however, ell has no complement at all, ie unit, then mapping l doesn't require one either. }
  ...
  appropriate simulation relation is defined by $
  \{(((i_k,g_k),(i_\ell,g_\ell)), (i_{k;\ell},g_{k;\ell}))\mid
  i_k=i_\ell=i_{k;\ell}\wedge \forall
  p. g_{k;\ell}(p)=(g_k(p),g_\ell(p))\} $.  We omit the
  straightforward verification.
%
  \iffull 
    We also have to show that $T(--)$ is well-defined on
    equivalence classes, so assume that $\ell\equiv k : X\lens Y$ and let
...
We also require that $\ell$ maps insertions to insertions, deletions
to deletions, and rearrangements to rearrangements. Note that this is
well-defined on equivalence classes of lenses.
...
{\infruleplain{T = \left<I,P,\live\right>\mbox{ a container type} \\
 ...
 { C &=& \ell.K
  ...
  \dputr_g(\mlinsert(\di),(i,c,i')) &=& (\mlrearrange(\ONE,f_i)\mlinsert(\di'),\\
&& (\di\ i, c',  \di'\ i'))\\
  \dputr_g(\mldelete(\di),(i,c,i')) &=& (\mlrearrange(\ONE,f_d)\mldelete(\di'), \\
&& (\di\ i, c', \di'\ i'))\\
  \dputr_g(\mlrearrange(\di,f),(i,c,i')) &=& (\mlrearrange(\di',f_r),\\
&&(\di\ i,c',\di'\ i'))\\[.8ex]
  && \text{see below for $f_i,f_d,f_r$}\\
...
definitions below.
%
The families of bijections $f_i, f_d, f_r$ must be chosen in such a
...
\end{array}\\
f_d(\di'\ i')(p) = f^{-1}_{i,c,i'}(f_{\di\ i,c',\di'\ i'}(p))\\
f_r(\di'\ i')(p) = f_{i,c,i'}^{-1}(f(i)(f_{\di\ i,c',\di'\ i'}(p)))
\end{array}\]

The propagated edits are supposed to be applied to a container of the
current shape $i'$, so these arbitrary decisions do not really matter;
...
The bijection $f_i$ contains a little more choice, namely the
behavior on the $T'$ positions in $f_{\di\ i,c',\di'\
  i'}^{-1}(\live(\di\ i)\setminus\live(i))$. Fortunately, they all
contain $\init_X$ so that the choice does not affect the resulting
state after application of the edit.
...
\end{theorem}
\begin{pf}
\mxh{No need to discuss preservation of equivalence!}
  ...
  $p\in\live'(i')$ so that $((i,f),(i,c,i'),(i',f'))$ are consistent.
  ...
  Case $\mlins \di$. Writing $(\di',c')=\ell.\dputr(\di,c)$ the new
  complement is $(\di\ i,c',\di'\ i')$. The resulting new states are
  $(\di\ i,f_1)$ and $(\di'\ i', f_1')$ where 
  \[\begin{array}{l}
    f_1(p)=\text{if }p{\in}\live(i)\text{ then }f(p)\text{ else }\init_X\\
    f_1'(p)=\text{if }p{\in}\live(i')\text{ then }f'(f_i(i')(p))\text{ else }\init_X 
  \end{array}
  \]
and the bijection $f_i(\di'\ i')\in\live'(\di'\ i')\simeq \live'(\di'\ i')$ satisfies
\[
f_{\di\ i,c',\di'\ i'}(p)\in\live(i)\Rightarrow f_i(\di'\ i')(p)= f^{-1}_{i,c,i'}(f_{\di\ i,c',\di'\ i'}(p))
\]
Let $p\in\live'(\di'\ i')$. We must show that $f_1(f_{\di\ i,c',\di'\
  i'}(p))=f_1'(p)$. If $f_{\di\ i,c',\di'\ i'}(p)\in\live(i)$ then
$f_1(f_{\di\ i,c',\di'\ i'}(p))=$ $f(f_{\di\ i,c',\di'\ i'}(p)) = $
$f'(f_{i,c,i'}^{-1}(f_{\di\ i,c',\di'\ i'}(p)))= $ $f_1'(p)$ where the
first equation uses the above characterization of $f_1$; the second
one uses consistency of $f$ and $f'$, and the third one uses the
characterizations of $f_1'$ and $f_i$.

Now  suppose that $f_{\di\ i,c',\di'\ i'}(p)\notin\live(i)$. In this case 
$f_i(\di'\ i')(p)\not\in \live'(i')$, since $f_i(\di'\ i')$ is a bijection. More precisely, 
if (for the sake of a contradiction) $q:=f_i(\di'\ i')(p)\in\live'(i')$ then 
$f_{i,c,i'}(q)\in\live(i)$ and $q=f_i(\di'\ i')(f_{\di\ i,c',\di'\ i'}^{-1}(f_{i,c,i'}(q)))$ so that 
$p = f_{\di\ i,c',\di'\ i'}^{-1}(f_{i,c,i'}(q))$ by injectivity and so
$f_{\di\ i,c',\di'\ i'}(p) = f_{i,c,i'}(q)\in\live(i)$.

If, as we have seen, $f_i(\di'\ i')(p)\not\in \live'(i')$ then
$f_1'(p)=\init_X$, but then $f_1(f_{\di\ i,c',\di'\ i'}(p))=\init_X$,
too, by the characterization of $f_1$.

We omit the other cases. 
\dmwit{proofneeded} % maybe the journal version should not omit the other cases
\end{pf}
\fi

We illustrate the propagation of an $\mlinsert(\di)$ edit in the
particular case where we are synchronizing a tree with the list formed by its
in-order traversal. Thus, $I=\mathbb{N}$; $P=\mathbb{N}$; $\live(i)=\{p\mid
p<i\}$ and $I'$ comprises prefix closed subsets of $\{0,1\}^*$;
$P'=\{0,1\}^*$; $\live'(i')=i'$. The monoid $\partial I$ has increment
...
\begin{wrapfigure}[7]{r}{1.3in}
\hspace*{-3em}\includegraphics[scale=.32]{images/restruct1.pdf} 
\end{wrapfigure}
...
\end{wrapfigure}
correspondence;
thus, for example if $i=4$ and $i'=\{\varepsilon,0,1,11\}$ the bijection
would be as shown above. (For illustration we also indicate possible
$X$-contents of the positions.)
%
Formally, we have $f_{i,c,i'} = \{(0,0), (1,\varepsilon), (2,1), (3,11)\}$. 
...
\{(0,00)$, $(1,0)$, $(2,01)$, $(3,\varepsilon)$, $(4,1)$, $(5,11)\}$ and
...
\begin{wrapfigure}[9]{r}{1.3in}
\hspace*{-4.3em}\includegraphics[scale=.32]{images/restruct3.pdf} 
\end{wrapfigure}
To restore consistency we also  apply $\mlrearrange(1,f_r)$ where 
$f_r(i')=\{(00,0)$, $(0,\varepsilon)$, $(01,1)$, $(\varepsilon,11)$,
$(1,00)$, $(11,01)\}$. 
We could also have chosen $f_r(i')= \{\dots, (1,01)$, $(11,01)\}$; this is
precisely the 
additional freedom of choice. Of course  $f_r(i'')$ for $i''\neq i'$ is also 
completely unconstrained. After applying $\mlrearrange(1,f_r)$ we end
up with the desired consistent state.
%% \begin{verbatim}
%% 0:be              eps:na
%% 1:da            /       \
%% 2:ma          0:da     1:?
...
commute:
\begin{align*}
    \mlleft\ONE &= \varepsilon \\
    \mlleft\dx\mlleft{\dx'} &= \mlleft{\dx\dx'} \\
    \mlright\ONE &= \varepsilon \\
    \mlright\dy\mlright{\dy'} &= \mlright{\dy\dy'} \\
...
To show that $\dputr$ respects the law $\mlleft\ONE=\varepsilon$:
\begin{align}
    \dputr(\mlleft\ONE,(c_k,c_\ell))
        ={}& \mllet (\dz,c_k')=k.\dputr(\ONE,c_k)\mline \nonumber\\
           & (\mlleft\dz,(c_k',c_\ell)) \nonumber\\
        ={}& (\mlleft\ONE,(c_k,c_\ell)) \label{eq:klens} \\
        ={}& (\varepsilon,(c_k,c_\ell)) \label{eq:law} \\
        ={}& (\ONE,(c_k,c_\ell)) \label{eq:one1} \\
        ={}& \dputr(\ONE,(c_k,c_\ell)) \nonumber\\
        ={}& \dputr(\varepsilon,(c_k,c_\ell)) \label{eq:one2}
\end{align}
...
assumption, and equations~\ref{eq:one1} and~\ref{eq:one2} follow because
$\ONE=\varepsilon$ in both modules.

Next we will show that $\dputr$ respects the law
$\mlleft\dx\mlleft{\dx'}=\mlleft{\dx\dx'}$. It will be convenient to name a
few things. Pick a state $c_k$ and define:
\begin{align*}
    (\dy',c_k') &= k.\dputr(\dx',c_k) \\
    (\dy,c_k'') &= k.\dputr(\dx,c_k')
\end{align*}
...
We may now compute:
\[\begin{array}{l}
    \dputr(\mlleft\dx\mlleft{\dx'},(c_k,c_\ell)) \\
    \qquad=
    ...
    \qquad=
        \mllet (\dy'',(c_k'',c_\ell'')) = \dputr_g(\mlleft\dx,(c_k',c_\ell)) \mline \\
    \qquad{\color{white}={}}
        (\dy''\mlleft{\dy'},(c_k'',c_\ell'')) \\
    \qquad=
        (\mlleft\dy\mlleft{\dy'},(c_k'',c_\ell)) \\
    \qquad=
        (\mlleft{\dy\dy'},(c_k'',c_\ell)) \\
    \qquad=
        \dputr_g(\mlleft{\dx\dx'},(c_k,c_\ell)) \\
    \qquad=
        \dputr(\mlleft{\dx\dx'},(c_k,c_\ell))
\end{array}\]

Finally, we show that $\dputr$ respects the law
$\mlleft\dx\mlright\dy=\mlright\dy\mlleft\dx$. As before, we choose a $c_k$
and $c_\ell$ and name a few intermediate computations:
\begin{align*}
...
Now we may compute:
\[\begin{array}{l}
\dputr(\mlleft\dx\mlright\dy,(c_k,c_\ell)) \\
    \qquad=
    ...
    \qquad=
        \mllet (\dx',(c_k'',c_\ell'')) = \dputr_g(\mlleft\dx,(c_k,c_\ell')) \mline \\
    \qquad{\color{white}={}}
        (\dx'\mlright{\dy'},(c_k'',c_\ell'')) \\
    \qquad=
        (\mlleft{\dx'}\mlright{\dy'},(c_k',c_\ell')) \\
    \qquad=
        (\mlright{\dy'}\mlleft{\dx'},(c_k',c_\ell')) \\
    \qquad=
        \dputr(\mlright\dy\mlleft\dx,(c_k,c_\ell))
\end{array}\]
...
\fi
Let $f$ be the isomorphism between $X \otimes \Unit$ and $X$. Similarly, let
$g$ be the obvious isomorphism between $\Unit \otimes Y$ and $Y$.%
%
\iffull
\footnote{Readers familiar with earlier work on symmetric state-based
  lenses~\cite{HofmannPierceWagner10} may observe that, unlike the analogous
  state-based lenses, these projections are \emph{not} parameterized by an
...
equations:
\begin{align*}
    \mlswitch_{jk}(m)\;\mlswitch_{ij}(m') &= \mlswitch_{ik}(m) \\
    \mlswitch_{ij}(m)\;\mlstay_i(m') &= \mlswitch_{ij}(m) \\
    \mlstay_j(m)\;\mlswitch_{ij}(m') &= \mlswitch_{ij}(mm') \\
    \mlstay_i(m)\;\mlstay_i(m') &= \mlstay_i(mm') \\
    d\;d' &= \fail \qquad \mbox{in all other cases}
\end{align*}
...
\begin{proof}
    ...
    \[\begin{array}{l}
        \mlswitchll m\mlswitchll{m'} \odot \mlinl x \\
        \qquad= \mlswitchll m\odot \mlinl{m'\odot \init} \\
        \qquad= \mlinl{m \odot \init} \\
        \qquad= \mlswitchll{m} \odot \mlinl x
    \end{array}\]

    \begin{align*}
        \mlswitchll m\mlstayl{m'} \odot \mlinl x
            &= \mlswitchll m \odot \mlinl{m'\odot x} \\
            &= \mlinl{m \odot \init} \\
            &= \mlswitchll m \odot \mlinl x
    \end{align*}

    \begin{align*}
        \mlstayl m\mlswitchll{m'} \odot \mlinl x
            &= \mlstayl m \odot \mlinl{m' \odot \init} \\
            &= \mlinl{m \odot m' \odot \init} \\
            &= \mlinl{mm' \odot \init} \\
            &= \mlswitchll{mm'} \odot \mlinl x
    \end{align*}

    \begin{align*}
        \mlstayl m\mlstayl{m'} \odot \mlinl x
            &= \mlstayl m \odot \mlinl{m' \odot x} \\
            &= \mlinl{m \odot m' \odot x} \\
            &= \mlinl{mm' \odot x} \\
            &= \mlstayl{mm'} \odot \mlinl x
    \end{align*}
\end{proof}
...
\emph{symmetric lenses}.  We refer to them here as \emph{state-based symmetric
  lenses}.
%
Recall from \cite{HofmannPierceWagner10} that a state-based symmetric lens $\ell$
...
Now, for any set $X$ we have the monoid $\partial X$ whose elements (edits)
are lists of elements of $X$ modulo the equality $xx=x$. An action of
$\partial X$ on $X$ is defined by $\varepsilon x=x$ and $(xw)y=x$
where $x\in X, w\in X^*$. Note that this is well defined as
$x(xy)=x=xy$. If, in addition, we have a distinguished element $x\in
X$, we thus obtain a module denoted $X_x$ where $|X_x|=X$ and
$\init_X=x$ and $\partial X_x = \partial X$.

Let  $\ell$ be  a state-based symmetric lens between $X$
and $Y$ along with elements $x\in X$ and $y\in Y$ satisfying
$\ell.\putr(x,\ell.\smissing)=(y,\ell.\smissing)$. 
%
We then define a symmetric edit lens $\partial_{xy} \ell$ between the
modules $X_x$ and $Y_y$ 
as follows: 
\iffull \begin{itemize} \fi
\iffull \item \else (1) \fi $(\partial_{xy}\ell).C = \ell.C$;
\iffull \item \else (2) \fi $(\partial_{xy}\ell).\missing = \ell.\smissing$;
\iffull \item \else (3) \fi $(\partial_{xy}
\ell).\dputr(\varepsilon,c)=(\varepsilon,c)$; 
\iffull \item \else (4) \fi $(\partial_{xy} \ell).\dputr(xw,c)=(yv,c'')$ where
  $(\partial_{xy}\ell).\dputr(w,c)=(v,c')$ and $\ell.\putr(x,c')=(y,c'')$;
\iffull \item \else (5) \fi analogous definitions for $\dputl$; and
\iffull \item \else (6) \fi $K = \{(x,c,y) \mid \ell.\putr(x,c)=(y,c)\}$%
\iffull \end{itemize} \else. \fi
$\partial_{xy} \ell$ is a symmetric edit lens and the passage from $\ell$
to $\partial \ell$ is compatible with the equivalences on symmetric
lenses and symmetric edit lenses. The definition for $\dputr$ is
well-defined because the round-trip law for symmetric lenses guarantees that
putting the same value twice in a row results in the same output both times,
...
\begin{theorem}
    ...
    $\partial_{xy}k\equiv\partial_{xy}\ell$.
\end{theorem}

\begin{proof}
Suppose $S$ is a witness that $k\equiv\ell$. Then we define
$\partial_{xy}S$ as follows:
\[\partial_{xy}S = \{(x,(x,c_k,y),(x,c_\ell,y),y) \mid (c_k,c_\ell) \in S
\land x \in X \land y \in Y\}\]

Since $(k.\smissing,\ell.\smissing)\in S$ and $k.\putr(x,k.\smissing)=(y,\_)$,
we know that:
\[\begin{array}{l}
    (\init_{X_x},(\partial_{xy}k).\missing,(\partial_{xy}\ell).\missing,\init_{Y_y}) \\
    \qquad= (x,k.\smissing,\ell.\smissing,y) \\
    \qquad\in \partial_{xy}S
\end{array}\]

It remains to show that $\partial_{xy}S$ is preserved by $\dputr$ and
$\dputl$; these arguments are very similar, so we focus on the one for
...
form a tuple in $\partial_{xy}S$. We proceed by induction on $\dx$.

In case $\dx=\varepsilon$, we are done: after computing $\dputr$, we still
have $x_0$, $y_0$, $c_k$, and $c_\ell$. Otherwise, $\dx = x_1\dx'$ and the
induction hypothesis tells us that if
$(\partial_{xy}k).\dputr(\dx',c_k)=(\dy_k,c_k')$ and
$(\partial_{xy}\ell).\dputr(\dx',c_\ell)=(\dy_\ell,c_\ell')$ then
$(c_k',c_\ell') \in S$ and $\dy_k=\dy_\ell$. We can then conclude that if
$k.\putr(x_1,c_k')=(y_k,c_k'')$ and
...
$(\partial_{xy}k).\dputr(\dx,c_k)=(y_k\dy_k,c_k'')$ and
$(\partial_{xy}\ell).\dputr(\dx,c_\ell)=(x_\ell\dx_\ell,c_\ell'')$. But we
have already seen that $y_k\dy_k=x_\ell\dx_\ell$ and $(c_k'',c_\ell'') \in
S$, so we are done.
\end{proof}
\fi
...
both equipped with differs, we define 
a symmetric lens $|\ell|$ between $|X|$ and $|Y|$ by 
\iffull \begin{itemize} \fi
\iffull \item \else (1) \fi $|\ell|.C = |X|\times \ell.C\times |Y|$;
\iffull \item \else (2) \fi $|\ell|.\missing = (\init_X,\ell.\missing,\init_Y)$;
\iffull \item \else (3) \fi $|\ell|.\putr(x,(x_0,c,y_0))= (\dy\ y_0,\, (x,
c',\dy\ y_0))$ where $\dx=\dif(x_0,x)$ and $(\dy,c')=\ell.\dputr(\dx,c)$;
and 
\iffull \item \else (4) \fi an analogous definition of $|\ell|.\putl$
\iffull \end{itemize} \fi
...
\begin{theorem}\label{eqchar}
 ...
 %
  The constructions $|-|$ and $\partial_{xy}$ then establish a
  ...
  between $X$ and $Y$, on the other.
\end{theorem}

\iffull
\begin{pf}
Let $\ell$ be a state-based lens between sets $X$ and $Y$ 
and let $\hat x\in X$, $\hat y\in Y$ satisfy
 $\ell.\putr(\hat x,\ell.\smissing)=(\hat y,\ell.\smissing)$. 
To show that $|\partial_{\hat x\hat y}(\ell)|\equiv \ell$ we use the 
simulation $R=\{((x,c,y),c')\mid \ell.\putr(x,c)=(y,c)\wedge c=c'\}$. 

Conversely, if $X$ and $Y$ are the modules $|X|_{\init_X}$ and
$|Y|_{\init_Y}$, respectively, and $\ell\in X\lens Y$ then $\ell\equiv
\partial_{\init_X,\init_Y}(|\ell|)$. To see this, we use the simulation 
$S=\{(x,c,(x',c',y'),y)\mid x=x',y=y',c=c'\}$. We omit the verification 
of both simulations.%
\end{pf}
\fi
