...
{\infruleplain{T = \left<I,P,\live\right>\mbox{ a container type} \\
 ...
 { C &=& \ell.K
  ...
  \dputr_g(\mlinsert(\di),(i,c,i')) &=&
      (\mlrearrange(\ONE,f_i)\mlinsert(\di'), \wideeqbreak
      (\di\;i, c', \di'\;i'))\\
  \dputr_g(\mldelete(\di),(i,c,i')) &=&
    (\mldelete(\di')\mlrearrange(\ONE,f_d), \wideeqbreak
    (\di\;i, c', \di'\;i'))\\
  \dputr_g(\mlrearrange(\di,f),(i,c,i')) &=&
    (\mlrearrange(\di',f_r), \wideeqbreak
    (\di\;i,c',\di'\;i'))\\[.8ex]
...
\end{array}\\
f_d(i')(p) &= f^{-1}_{i,c,i'}(f_{\di\ i,c',\di'\ i'}(p))\\
...
\begin{pf}
  ...
  $p\in\live'(i')$ so that $((i,f),(i,c,i'),(i',f'))$ are consistent. In
  the cases below where there is an edit named $\di$, we will write
  $(\di',c')=\ell.\dputr(\di,c)$ and abbreviate the bijections $f_{i,c,i'}$
  and $f_{\di\;i,c',\di'\;i'}$ to $f_{pre}$ and $f_{post}$, respectively.
  ...
  \end{align*}
Also, the bijection $f_i(\di'\ i')\in\live'(\di'\ i')\simeq \live'(\di'\ i')$ satisfies
\[
...
if $f_i(\di'\;i')(p) \in \live'(i')$. In the forward direction, we argue:
\begin{align*}
...
$f_1(f_{post}(p))=\init_X$, too, by the characterization of $f_1$.
  ...
  are $(\di\;i,f)$ and $(\di'\;i',f_d(i');f')$. For these to be consistent,
  we must show that $f(f_{post}(p)) = f'(f_d(i')(p))$ whenever $p \in
  ...
  \begin{align*}
      ...
      &= f'(f_d(i')(p)) &&\mbox{assumed condition of }f_d
  \end{align*}
