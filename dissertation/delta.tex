...
\begin{pf}
  ...
  complement is $(\di\ i,c',\di'\ i')$. Let us abbreviate the two bijections
  $f_{i,c,i'}$ and $f_{\di\;i,c',\di'\;i'}$ to $f_{pre}$ and $f_{post}$,
  respectively. The resulting new repository states are $(\di\ i,f_1)$ and
  $(\di'\ i', f_1')$ where
  \begin{align*}
...
\[
f_{post}(p)\in\live(i)\Rightarrow f_i(\di'\ i')(p)= f^{-1}_{pre}(f_{post}(p)).
\]

Our current assumptions let us conclude that $f_{post}(p) \in \live(i)$ if
and only if $f_i(\di'\;i')(p) \in \live'(i')$. In the forward direction, we
argue:
\begin{align*}
    f_{post}(p) &\in \live(i) &&\mbox{assumption} \\
    f_{pre}^{-1}(f_{post}(p)) &\in \live'(i') &&f_{pre} \in \live(i) \simeq
    \live'(i') \\
    f_i(\di'\;i')(p) &\in \live'(i') &&\mbox{assumed condition of }f_i
\end{align*}
In the backward direction, define $q=f_i(\di'\;i')(p)$. Then:
\begin{align*}
    f_i(\di'\;i')(p) &\in \live'(i') &&\mbox{assumption} \\
    ...
    f_i(\di'\;i')(f_{post}^{-1}(f_{pre}(q))) &= q
        &&\mbox{assumed condition of }f_i \\
    f_{post}^{-1}(f_{pre}(q)) &= p
        &&f_i(\di'\;i')\mbox{ is a bijection and} \\
...
and $f'$, and the third one uses the characterizations of $f_1'$ and $f_i$
(noting that $f_{post}(p) \in \live(i)$ implies $f_i(\di'\;i')(p) \in
\live'(i')$). In the other case, $f_{post}(p) \notin \live(i)$, hence
$f_i(\di'\;i')(p) \notin \live'(i')$. Then $f_1'(p)=\init_X$, and
