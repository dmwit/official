...
% TODO: some proofneeded's are left
...
\begin{theorem}\ 
    \begin{itemize}
        ...
        \item $(k \oplus \ell);(k' \oplus \ell') \equiv (k;k') \oplus
            (\ell;\ell')$.
        \item $\big(k \oplusl \ell\big);\bij_{\swap} \equiv \ell \oplusr k$, where
            $\swap$ is the obvious module isomorphism between $X \oplusl Y$
            and $Y \oplusr X$.
    \end{itemize}
\end{theorem}
...
\begin{pf}\ 
    \begin{itemize}
        ...
        \item $\big(k\oplusl\ell\big);\bij_{\swap}\equiv\ell\oplusr k$

            We give a witnessing relation that rearranges the bits of the
            complements:
            \[S = \{(x,(\swap(c),\unit),c,y) \mid (x,c,y) \in (\ell\oplusr
            k).K\}\]
            As in previous proofs, we will name the two lenses in question
            $a=\big(k\oplusl\ell\big);\bij_\swap$ and $b=\ell\oplusr k$.
            Supposing that the types are $k \in X_k \lens Y_k$ and $\ell \in
            X_\ell \lens Y_\ell$, we first observe that
            \begin{align*}
                \multicolumn{2}{l}{$\left(\init_{X_\ell\oplusr X_k}, a.\missing,
                b.\missing, \init_{Y_\ell\oplusr Y_k}\right)$} \\\qquad
                &= \left(\mlinr{\init_{X_k}}, (\mlinl{k.\missing},\unit),
                    \mlinr{k.\missing}, \mlinr{\init_{Y_k}}\right) \\
                &= \left(\mlinr{\init_{X_k}},
                    (\swap(\mlinr{k.\missing}),\unit), \mlinr{k.\missing},
                    \mlinr{\init_{Y_k}}\right) \\
                &\in S
            \end{align*}
            because $(\init_{X_k}, k.\missing, \init_{Y_k}) \in k.K$. It
            only remains to show that $S$ is preserved by $\dputr$ and
            $\dputl$ applied to applicable edits.
