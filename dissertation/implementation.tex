...
%   * can't be performant with current signature
%   * ad-hoc, doesn't always work
%   * is compositional, but matching lenses shows us that we don't always
%     want compositional
% * possible ways forward
% TODO: possible way to explain why incremental parsers aren't it: they are
% focused on improving efficiency, but do not attempt to track
% provenance/alignment; so edits are used to find the smallest region that
% needs to be reparsed but their meaning is otherwise ignored
