...
all of our results in Coq from an experimentation point of view, the
availability of dependent types would have made some parts of the
implementation more straightforward. For example, we used Haskell's
typeclass mechanism to model modules, which requires a separate type for
each separate behavior. As a result, one wants the ability to define a new
types for each choice of $\init$ value, that is, a dependent type. We
...
Our primary challenge, which we will discuss in detail below, is analogous
