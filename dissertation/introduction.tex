...
like the two repositories to stay ``in sync'': modifications to one
...
Table~\ref{tab:related-work-matrix} compares extant lens frameworks with
respect to these four key features, with a focus on frameworks which aim at
dealing with the problem of alignment. We will discuss this figure in depth
in Chapter~\ref{chap:related}; for now, it suffices to observe that the edit
...
%        triple graph grammars   &\N&\Y&\N&\Y\\
        ...
        comma category lenses   &\Y&\N&\N&\N\\
        group-based lenses      &\Y&\N&\N&\N\\
        matching lenses         &\Y&\N&\N&\Y\\
        annotation-based lenses &\Y&\N&\N&\Y\\
...
introduction to the core concepts. Suppose there are two repositories; one
...
Lenses have one more piece, which was alluded to above. The structure
described so far already addresses the need to give two transformations
...
phrase all our desires in terms of constraints on $\ell.\aput$ and
$\ell.\acreate$. It is
...
step in refining this behavioral law. The third law, \rn{CreateGet}, serves
a similar purpose to the \rn{PutGet} law.
...
operation, it is important to carefully address the behavior of the
bidirectional \map (which we will denote by $-\LIST$ to distinguish it from
the unidirectional version), and that turns out to be surprisingly
...
value from each repository, the $\ell\LIST.\aput$ operation takes two lists,
of types $S\LIST$ and $V\LIST$. When these lists are the same length they can
just be zipped together, applying $\ell.\aput$ to pairs of elements in the
...
\begin{figure}
    \begin{align*}
        ...
        \tolower.\acreate(c) &= c
    \end{align*}
    \caption{The \tolower lens converts a possibly-upper-case letter into
...
Additionally, many lens frameworks treat such alignment information
...
designing a well-behaved $-\LIST$ combinator involves rethinking some or
...
Continuing the example from \ref{sec:intro-alignment}, suppose the school
secretary decided to begin tracking which room each teacher uses. The
...
giving a value to return from the \CREATE operation, but these annotations
are elided to avoid clutter.)
...
operations: first $\pi_{1,3}\LIST.\aput$ to update the common
...
system synchronization tools like rsync, DropBox, and Unison; collaborative
document editing tools like Apache Wave and Google Docs; and revision
control systems like CVS, SVN, darcs, mercurial, and git~%
\cite{Tridgell96rsync,dropbox,pierce2004unison,berliner1990cvs,subversion,darcs,mercurial,git,ApacheWave,GoogleDocs},
...
work and its closest variants is the devotion to retaining a large
collection of lenses and lens combinators that have the appropriate types
and behavior for the given lens framework. This feature---which we will call
a \emph{syntax}---is well worth emulating, for several reasons. The
...
supporting it involves relaxing the restriction that a single pass of the
...
alignment issues. Edit lenses occupy a unique niche in
...
Finally, Chapter~\ref{chap:conclusion} summarizes the contributions made
by our work. We will review the techniques used to address each of the four
challenges raised here, then go on to suggest a collection of additional
challenges which could be the basis for future research: multi-repository
lenses, a syntax based on linear logic, further algebraic study, application
of the theory, and several other miscellaneous extensions. There are many
opportunities for an extended theory of lenses; nevertheless, our work is
already a step in a new and important direction.
...
\begin{table}
    ...
    \begin{tabular}{r@{}c@{}l|l}
        ...
        $X$ & ${}\lens     {}$ & $Y$ & (Chapter~\ref{chap:edits}) symmetric, edit-based lenses connecting $X$ and $Y$
        % TODO: is this really all the kinds of arrows we have everywhere?
        % TODO: at least the following are missing: adlens, sdlens, clens
    \end{tabular}
