...
areas where two pieces of data are very similar. In all cases, one would
...
two programs that work together. Calling the two pieces of data $A$ and $B$,
the first program translates updates to $A$ into updates to $B$, and the
second translates in the other direction, turning updates to $B$ into
updates to $A$. (Taking the example of connecting a piece of source code and
...
\begin{itemize}
    ...
    \item $V$, $W$, $X$, $Y$, $Z$ for the values synchronized by symmetric
        lenses,
...
We will also need to deal with infinite lists, which we denote $X\INFTY$
when the elements are drawn from $X$. The infinite list with $x_i$ in the
$i$th position is written $\blist x_1 \ilist$, and the infinite list
