...
descriptive tags).
% TODO: oh, suddenly this is going to be a publication, so we need to be
...
\begin{figure}
    \begin{diagram}
        \draw[short picture tree]
            ...
            ;
        \lolcattagsdef{froghead-fs-pic}
            {palindrome/{costume,food},gamer/{anthro},froghead/{costume}}
    \end{diagram}
...
to lists by extracting the leaves, and a $\mathit{retag}$ maintainer that
...
which require a $B$ is input. One can keep the system working by keeping a
copy of the ``intermediate'' repository around somewhere and running the
constraint maintainers in sequence on each update, but this steps outside
the framework of constraint maintainers.
...
\begin{figure}
    \begin{diagram}
        ...
        \begin{scope}[start chain=going below,node distance=0]
            \foreach \name in {palindrome,gamer,froghead} { \draw
                (root.north -| froghead-fs-pic) +(3,0)
                node[on chain,anchor=north]    (\name-inter-pic)  {\lolcat{\name}}
                node[right=of \name-inter-pic] (\name-inter-name) {\tiny \name.jpg}
                ;
            }
        \end{scope}
        \lolcattags{(palindrome-inter-pic.north -| palindrome-inter-name.east) +(2,0)}
            {palindrome/{costume,food},gamer/{anthro},froghead/{costume}}
        \node[draw,rounded corners,fit=(root)
            (palindrome-fs-name) (palindrome-fs-pic)
              (froghead-fs-name)   (froghead-fs-pic)
             ] (fs) {};
        \node[draw,rounded corners,fit=
            (palindrome-inter-pic) (palindrome-inter-name)
              (froghead-inter-pic)   (froghead-inter-name)
             ] (inter) {};
        \node[draw,rounded corners,fit=
            (palindrome-web-pic) (palindrome-web-tag)
              (froghead-web-pic)   (froghead-web-tag)
             ] (web) {};
        \draw[<->] ($(fs.south)+(0,-0.1)$)
            to[bend right] node[below] {$\mathit{flatten}$}
            ($(inter.south)+(-0.5,-0.1)$);
        \draw[<->] ($(inter.south)+(0.5,-0.1)$)
            to[bend right] node[below] {$\map(\mathit{retag})$}
            ($(web.south)+(0,-0.1)$);
    \end{diagram}
...
Ideally, one would like a model that captures the behavior of these
``sequenced'' chains, retains a symmetric presentation, and allows each
repository to retain information not available in the other.
