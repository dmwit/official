...
TODO: perhaps we'd like a little discussion about the roles of semantics and
syntax, and how they relate
...
\begin{itemize}
    ...
    \item $V$, $W$, $X$, $Y$, $Z$ for symmetric lens repositories
    \item $C$ for complement sets
    \item lower case letters for members of upper case letters
    \item $\DX$, $\DY$, $\DZ$, etc. for sets of edits to $X$, $Y$, $Z$, etc.
    \item $\dx$, etc. for members of $\DX$, etc.
    \item $\ml{sans}$ for code; \texttt{typewriter} for data;
        \textsc{SmallCaps} for law names
    \item $X\LIST$ for the set of lists with elements drawn from $X$;
        $X\INFTY$ for the set of infinite lists with elements drawn from $X$
    \item $\NIL$ for empty lists, $x \CONS t$ for the list that starts with
        $x$ and ends with $t$, $\left<x_1,\ldots,x_n\right>$ for the
        length-$n$ list with value $x_i$ in the $i$th element; will often
        use the variable with no subscript to denote the whole list
    \item we conflate elements $v$ and single-element lists $\blist v
        \elist$ when it is clear from context
    \item $|x|$ is the length of the list $x$
    \item $x[i\mapsto v]$ is
        $\left<x_1,\ldots,x_{i-1},v,x_{i+1},\ldots,x_n\right>$
    \item $\unit$ for the single element of the singleton set $\Unit$
    \item record syntax for lens definitions/uses
    \item $A \to B$ vs. $A \lens B$ vs. $A \alens B$ vs. $A \cmaint B$ vs. $A \dlens B$
    \item \emph{repository}: a collection of data
\end{itemize}
...
Y$, we will have $\map(\ell) \in X\LIST \alens Y\LIST$ where $X\LIST$ is the type of
...
repository, so again in the case where $\ell \in S \alens V$, we will have
$\map(\ell).\aput$ taking two lists, one each of types $S\LIST$ and $V\LIST$.
