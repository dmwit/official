    ...
    \begin{diagram}
        \draw[short picture tree]
            node (root) {} child {
                ...
                node (May) {May}
                    \lolcatchildren{mechanic}
            }
            ;
        \lolcattagsdef{mechanic-fs-pic}
            {pumpkin/{costume},skateboard/{anthro,vehicle},mechanic/{vehicle}}
    \end{diagram}
    % TODO: citations for cat pictures somehow; see the images/provenance file
...
\begin{figure}
    \begin{diagram}
        ...
        \begin{scope}[start chain=going below,node distance=0]
            \foreach \name in {pumpkin,skateboard,mechanic} { \draw
                (root.north -| mechanic-fs-pic) +(3,0)
