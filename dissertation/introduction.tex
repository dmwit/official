...
frameworks rule this transformation illegal, because supporting it involves
relaxing the restriction that a single run of the synchronization tool
produces a synchronized state. Whether one prioritizes behavioral guarantees
or richer syntax may be a matter of taste; but the choice would not be
readily apparent without attempting to design the syntax in parallel with
the semantics.

Finally, the design of a collection of instantiations is a critical proving
ground for the practicality of other features. A good lens framework can
have the potential to solve alignment, symmetry, or performance problems,
but only an instantiation can realize or dispel that potential.
