...
% TODO: could use some more color/highlighting
\begin{frame}
    ...
    \begin{diagram}
        \draw
            ...
            node[anchor=south west] (parsed before bot) {(Hayn, 1732)}
            ($(parsed before top.west)!.5!(parsed before bot.west)$)
            node[anchor=west]       (parsed before mid) {(Bach, 1675)}
            ...
            node[anchor=south west] (parsed after bot) {(Haydn, 1732)}
            ($(parsed after top.west)!.5!(parsed after bot.west)$)
            node[anchor=west]       (parsed after mid) {(Bach, 1685)}
            ...
            (raw before) edge[->]
                node[right,pos=0.4]  {\tiny Delete 24 30}
                node[right,pos=0.53] {\tiny Insert \ 24 ``85\textbackslash nHayd''}
            (raw after)

            (parsed before bot) edge[->]
                node[right,pos=0.4]  {\tiny mod(1,right(1685))}
                node[right,pos=0.53] {\tiny mod(2,left(Haydn))}
