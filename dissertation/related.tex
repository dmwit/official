...
edit translation morphism $f \in \DS \to \DV$ may be extended to a short exact
sequence. The morphism $s \in \DV \to \D\Unit$ must be the constantly-$\ONE$
...
morphism $i$ is completely determined by the morphism laws once we have
...
\begin{definition}
    ...
    there is a morphism $g\in\DV\to\DS$ such that $g;f = \id_{\DV}$.
\end{definition}
...
\PUT function. Then $g;f = \id_{\DV}$ says that all the information available in
$V$-edits are also available in $S$-edits, an analogous restriction to the
