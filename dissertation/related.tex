...
\Mod(G)=G_1\LIST/{\sim}_;$ be the set of paths quotiented by the
...
The empty list serves as the identity. Edit application is given by
\begin{align*}
    \NIL \odot x &= x \\
    (\dx \CONS t) \odot x &= x' & \mbox{when }\dx : (t \odot x) \dedge x'
\end{align*}
and undefined if the side-condition of the second case is not met or is
undefined. One may choose any object to play the role of $\init$ (so that
...
\lensdef{delta_symmetrize}
    ...
    }
    {
        ...
        \dputl_g(\dv, \mlinl s) &=& \mllet \ds = \ell.\aput_1(\dv)(s) \mlinm (\ds, \ds\odot_t s) \\
        && \mbox{when }\mldom(\dv)=\ell.\aget_0(s) \\
        \dputl_g(\dv, s) &=& (\fail,\mlinr \fail) \mbox{ otherwise}
    }
