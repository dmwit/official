...
follows. To pass from a module $X$ to a graph $\Gr(X)$, let $\Gr(X)_0=|X|$ be the set
of nodes and $\Gr(X)_1=\{m : x \dedge y \mid mx=y\}$ be the set of edges (so that
...
a category $G$ to a module $\Mod(G)$. (An arbitrary reflexive graph may be turned
...
paths.) Let the values $|\Mod(G)|=G_0$ be the set of nodes, and edits
$\D \Mod(G)=\{\fail\}\cup G_1/\sim_\id$ be the set of edges quotiented by the
