...
\begin{definition}
    ...
    \end{itemize}
    such that if $(y,r') = \mlfppg(x,r)$ then $r : \mldom(x) \uedge \mldom
    (y)$ and $r' : \mlcod(x) \uedge \mlcod(y)$ (and similarly for
    $\mlbppg$).
\end{definition}
...
\begin{definition}
    ...
    relation $r$ for $\ell$, we define equivalence relations:
    \begin{align*}
        \eqsharedl*\ell r &={} \under_{\lambda x.\;\ell.\mlfppg(x,r)} \\
        \eqsharedr*\ell r &={} \under_{\lambda y.\;\ell.\mlbppg(y,r)}
    \end{align*}
    When the lens is understood from context, we will write $x\eqsharedl
    rx'$ instead of $x\eqsharedl*\ell rx'$ (and similarly for $y\eqsharedr
    ry'$).
\end{definition}
