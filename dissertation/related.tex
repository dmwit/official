...
\begin{definition}
    ...
    \begin{align*}
        G \domco H &= \{(e_g,e_h) \mid e_g : v \dedge v' \land e_h : v
        \uedge v''\} \\
        G \codco H &= \{(e_g,e_h) \mid e_g : v \dedge v' \land e_h : v'
        \uedge v''\}
    \end{align*}
\end{definition}

\begin{definition}
    A \emph{symmetric delta lens} $\ell$ connecting model spaces $A$ and
    $B$, written $\ell \in A \sdlens B$, consists of:
    \begin{itemize}
        \item a bipartite graph $R$ whose two parts are $A_0$ and $B_0$ (the
            edges of $R$ are called \emph{correspondence relations}),
        \item a function $\mlfppg \in A \domco R \to B \codco R$,
            and
        \item a function $\mlbppg \in B \domco R \to A \codco R$
    \end{itemize}
    such that if $(b,r') = \mlfppg(a,r)$ then $r : \mldom(a) \uedge \mldom
    (b)$ and $r' : \mlcod(a) \uedge \mlcod(b)$ (and similarly for
    $\mlbppg$).
\end{definition}

The preconditions for $\mlfppg$ above stating that the edit and
correspondence relations are source-coincident and the postconditions saying
that the two edits and two correspondence relations together make a
well-typed square are somewhat similar to our precondition requiring an edit
which applies cleanly and postconditions saying that we produce an
applicable edit that restores consistency.
