...
Rule \rn{LL1} amounts to saying that $f$ is a module homomorphism (not just
a monoid homomorphism). Rule \rn{LL2} is a bit more subtle, but is motivated
by this rephrasing of the state-based asymmetric lens framework's
\rn{GetPut} law:
\infrule[GetPutAlt]
    {\aget(s)=v}
    {\aput(v,s)=s}
...
does not change the current view $f(\ds)\;\init_V$, then the edit we output
should not change the current source $\ds\;\init_S$.''

% TODO: high points of the work once we get past the definitions:
% * a lens-like sequence + a diff operation -> a state-based lens
% * take \DV to be the monoid of invertible endofunctions on V; can define a
%   submonoid \DS of endofunctions on S and turn a vwb lens into a lens-like
%   sequence on groups
% * lens -> lens-like sequence -> lens = id
% * close relationship between inverse edits and PutPut
...
% * we have partial actions; they have total ones
% * should think about what this condition of having g;f=id means, and whether
%   we have an analog
% * their lens-like requirement seems too strong (e.g. it's not clear to me
%   that if an edit happens to do nothing on one side it should be
%   translated to an edit which happens to do nothing)
% * we have way more syntax
% * we haven't thought much about groups at all
% * asymmetric vs. symmetric, of course
