...
Figure~\ref{fig:school-salaries-keys} shows the bad behavior of positional
alignment.
...
\begin{figure}
    \begin{diagram}
        \draw
            ...
            ($(V'-2-1.west) +(-0.5em,0)$) edge[reorder, bend right=60] ($(V'-4-1.west) +(-0.5em,0)$)
            (V'-3-1.west)                 edge[reorder, bend left]     (V'-2-1.west)
            (V'-4-1.west)                 edge[reorder, bend left]     (V'-3-1.west)
            ...
            ;
    \end{diagram}
    \caption{An easily fixed misalignment, and a more subtle one}
