...
\begin{definition}
    ...
    \begin{itemize}
        ...
        \item Arrows are pairs $(g,g')$ of an arrow $g : s \to s'$ from $S$
            and an arrow $g' : v \to v'$ from $V$ that forms a commuting
            square:
            \begin{diagram}
                \commaarrowpathdef
            \end{diagram}
...
\begin{definition}
    ...
    \infax[CPutGet]{\aget\left(\aput\left(\commaarrowdef\right)\right) = g'}
    \infax[CGetPut]{\aput\left(\commaarrow s{\aget(\id)}{\aget(s)}{s'}{\aget(\id)}{\aget(s')}g{\aget(g)}\right)=g}
    \infrule[CPutPut]
        {\aput\left(\commaarrow sgu{s'}{g'}{u'}f{f'}\right)=p : t \to t'}
        {
        \aput\left(\commaarrow thv{t'}{h'}{v'}{p}{f''}\right)
        =
        \aput\left(\commaarrow s{g;h}v{s'}{g';h'}{v'}f{f''}\right)
        }
\end{definition}
...
$\aget(\aput(f : \aget(s) \to v))=v$, that is, if we update the view to $v$,
...
once: the computation $\aput(g;h : \aget(s) \to v)$ should have the same
result as computing $\aput(g : \aget(s) \to u)$ (producing a source state
$t$ for which $\aget(t)=u$) and then $\aput(h : \aget(t) \to v)$. This is
