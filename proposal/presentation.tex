...
\usepackage{presentation}\lastframe{40}
\usepackage{amssymb}
\usepackage{semantic}
...
\begin{frame}
    ...
    \begin{center}
        \begin{tikzpicture}
            \draw[node distance=3em]
                node[lens]                          (const) {const}
                node[replica,right of=const,node distance=4em] (rate) {rate}
                node[lens,right of=rate]            (times) {$*$}
                node[replica,above right of=times]  (base)  {base}
                node[replica, below right of=times] (tax)   {tax}
                node[lens,below right of=base]      (plus)  {$+$}
                node[replica,right of=plus]         (price) {price}
                (rate) -- (times) -- (base) -- (plus) -- (price)
                                     (times) -- (tax)  -- (plus)
                ;
            \temporal<2>{\draw}{\draw[->,important]}{\draw[->]}
                (const) -- (rate);
            \temporal<2>{\draw}{\draw[->,important]}{\draw[->]}
                (rate) -- (times);
            \temporal<2>{\draw}{\draw[->,important]}{\draw[->]}
                (price) -- (plus);
            \alt<1-2>{\draw}{\draw[->,important]}
                (plus) -- (base);
            \temporal<3>{\draw}{\draw[->,important]}{\draw[->]}
                (base) -- (times);
            \temporal<3>{\draw}{\draw[->,important]}{\draw[<-]}
                (times) -- (tax);
            \temporal<3>{\draw}{\draw[->]}{\draw[<-,important]}
                (tax) -- (plus);
        \end{tikzpicture}
    \end{center}
    \pause[4]
...
% TODO: extra slide explaining what we know about linear equations

\begin{frame}
    \frametitle{How to progress}
    \begin{center}
        \begin{tikzpicture}[font=\tiny]
            \draw
                node[lens]                     (l) {$\ell$}
                node[replica,left of=l]        (A) {$A$}
                node[replica,above right of=l] (B) {$B$}
                node[replica,below right of=l] (C) {$C$}
                (l) -- (A)
                (l) -- (B)
                (l) -- (C)

                node[fit=(A) (B) (C)] (constraint) {}
                (constraint.south)
                node[anchor=north] {\normalsize constraint\strut}
                ;
        \end{tikzpicture}
        \hfil
        \begin{tikzpicture}[font=\tiny]
            \draw
                node[lens]                          (l-1) {$\ell$}
                node[replica,left of=l-1]           (A-1) {$A$}
                node[replica,above right of=l-1]    (B-1) {$B$}
                node[replica,below right of=l-1]    (C-1) {$C$}

                node[lens,right of=l-1,node distance=5em]
                                                    (l-2) {$\ell$}
                node[replica,left of=l-2]           (A-2) {$A$}
                node[replica,above right of=l-2]    (B-2) {$B$}
                node[replica,below right of=l-2]    (C-2) {$C$}

                node[fit=(A-1) (B-2) (C-2)] (methods) {}
                (methods.south)
                node[anchor=north] {\normalsize sample methods\strut}
                ;
            \draw[->] (A-1) -- (l-1);
            \draw[->] (l-1) -- (B-1);
            \draw[->] (l-1) -- (C-1);
    ...
    \end{center}
    \begin{itemize}
        \item When any plan will do: greedy algorithm
        \item Assign a cost to each method
        \item Specification: min-cost set of methods
        \item Implementation: search (efficient when combining costs is
            monotonic)
    \end{itemize}
\end{frame}
