...
\begin{frame}
    ...
    \begin{align*}
        get(put(y,x))&=y \\
        put(get(x),x)&=x \\
        \uncover<3>{\color{gray}put(y_2,put(y_1,x))&\color{gray}=put(y_2,x)}
    \end{align*}
    ...
    \begin{center}
        \begin{tikzpicture}[node distance=3em]
            \draw
                ...
                node[replica,above left of=center]  (i1-left)  {$I_1$}
                node[replica,below left of=center]  (i2-left)  {$I_2$}
                ...
                node[replica,above right of=center] (i1-right) {$I_1$}
                node[replica,below right of=center] (i2-right) {$I_2$}
                node[lens,below right of=i1-right]  (l)        {$\ell$}
                node[replica,right of=l]            (o2)       {$O_2$}
                (l) -- (i1-right)
                (l) -- (i2-right)
                (l) -- (o2)

                node[below=3em of center]            (makes)     {$\Downarrow$}
                node[replica,below=0.2em of makes]   (i1-bottom) {$I_1$}
                node[lens,below of=i1-bottom]        (kl)        {$k;\ell$}
                node[replica,below left of=kl]       (o1-bottom) {$O_1$}
                node[replica,below of=kl]            (i2-bottom) {$I_2$}
                node[replica,below right of=kl]      (o2-bottom) {$O_2$}
                (kl) -- (i1-bottom)
                (kl) -- (i2-bottom)
                (kl) -- (o1-bottom)
                (kl) -- (o2-bottom)
    ...
    \begin{center}
        \begin{tikzpicture}[node distance=3em]
            \path
                ...
                node[replica,below right of=k] (i2) {$I_2$}
                node[replica,left of=k] (o1) {$O_1$}
...
% TODO: related work (though how is the flow going to work? should it really
% go here, or earlier, or later, or... dunno lol)
\begin{frame}
